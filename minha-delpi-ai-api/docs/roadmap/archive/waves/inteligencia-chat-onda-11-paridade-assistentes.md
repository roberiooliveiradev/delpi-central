...
| Pedido | Documento |
|--------|-----------|
| Canvas / tabelas / gráficos estilo ChatGPT (25/mai) | [apresentacao-rica-chat-onda-9.md](../misc/apresentacao-rica-chat-onda-9.md) — **concluída** |
| Auditoria rota a rota api-delpi | [api-delpi-chat-intelligence-audit.md](../audits/api-delpi-chat-intelligence-audit.md) |
...
| 11.2.2 | Calibrar `LLM_MAX_TOKENS` / `OLLAMA_NUM_CTX` por ambiente (homologação &lt; 15s) | ✅ | `CHAT_LLM_LATENCY_PROFILE` (`operational_cpu`, `balanced`, `documental`); checklist em [rag-context-min-score-calibracao.md](../misc/rag-context-min-score-calibracao.md) |
...
| # | Item | Status | Critério de pronto |
|---|------|--------|-------------------|
| 11.6.1 | Tool interna `web_search` + policy de fontes | ✅ | Retry EN, provedores Tavily/Serper/Bing/**SearXNG**, sanitização de query, fallback Wikipedia PT, síntese LLM multi-seção, painel **Fontes · N** com atividade de pesquisa |
...
**Ainda depende de plataforma (fora deste repo):** perfis RBAC centralizados no `core-api` ([melhorias-futuras.md](../../active/melhorias-futuras.md)); provider api-delpi habilitado no agente para actions OpenAPI das rotas 5S.
...
| `CHAT_LLM_LATENCY_PROFILE` | `operational_cpu` | Preset `LLM_MAX_TOKENS` + `OLLAMA_NUM_CTX` (`320` / `1024`); ver [`rag-context-min-score-calibracao.md`](../misc/rag-context-min-score-calibracao.md) |
...
| `CHAT_WEB_SEARCH_SEARXNG_BASE_URL` | — (dev compose: `http://searxng:8080`) | Meta-busca OSS self-hosted; habilitar JSON em `infra/searxng/settings.yml` |
| `CHAT_WEB_SEARCH_SEARXNG_LANGUAGE` | `pt-BR` | Idioma da consulta SearXNG |
| `CHAT_WEB_SEARCH_SEARXNG_CATEGORIES` | `general` | Categoria SearXNG |
| `CHAT_WEB_SEARCH_TAVILY_API_KEY` | — | Tavily Search API (recomendado em prod / e-commerce) |
| `CHAT_WEB_SEARCH_SERPER_API_KEY` | — | Serper (Google) |
| `CHAT_WEB_SEARCH_BING_API_KEY` | — | Bing Web Search v7 |
| `SEARXNG_BASE_URL` | `http://localhost:8088/` | URL pública do container SearXNG (dev, porta host 8088) |
...
|-------|-----------|------|
| **12A** | [Onda 12 — Análise de desenhos PDF](../../active/inteligencia-chat-onda-12-skill-analise-desenhos-pdf.md) | Skill `drawing-analyser` herdável; PDF × API DELPI × checklist (paridade ChatGPT DELPI legado) |
