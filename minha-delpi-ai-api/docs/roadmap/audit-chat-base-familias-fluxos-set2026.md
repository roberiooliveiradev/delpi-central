...
**Saída:** `docs/operations/human-interaction-battery-eval.json` — por caso: `status`, `ms`, `evidence.R1…R8`, `paths`, `operationIds`. Copiar trechos para a planilha § 5.
...
*(Bateria interação humana: `docker exec -e SMOKE_BASE_URL=http://delpi-gateway -w /app delpi-minha-delpi-ai-api python scripts/human_interaction_battery_live.py` → JSON em `docs/operations/human-interaction-battery-eval.json`.)*
...
| Bateria interação humana simulada | **este doc § 1.3**; `scripts/human_interaction_battery_live.py` |
...
| JSON bateria humana | `docs/operations/human-interaction-battery-eval.json` |
| Avaliação humana A–D (referência) | `docs/operations/smoke-packages-a-d-human-evaluation.md` |
...
3. Revisar resumo no terminal + `docs/operations/human-interaction-battery-eval.json`.
