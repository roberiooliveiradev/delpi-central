...
|---------|---------|-------------------|
| `external_action_responses.json` | **`domainPredicates`** + **`systemPredicates`** + **`productSearchPredicates`** + `actionSelection` (incl. **`manifestText`** — caps/labels do manifesto OpenAPI) + `selectionReasons` | `ChatProductRoutePredicateService`, `ExternalActionResponseContentService`, `ExternalActionManifestTextService` |
