...
| `drawing_stamp.json` | Rótulos de carimbo, regiões, **`layoutAnalysis`** (XY-Cut), exclusões cliente/BOM, clarificações, gate nativo (Onda 14), **padrões de cota/decape** | `ChatDrawingStampExtractionService`, `ChatDrawingPageLayoutAnalysisService`, `ChatDrawingDimensionsExtractionService`, `ChatDrawingPatternsService` |
