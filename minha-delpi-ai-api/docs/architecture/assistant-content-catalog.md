...
| `response_modes.json` | Modos Rápida/Normal/Pensador (rótulos, aliases, `pipelineEffects`, **`fastCommentaryDirect`**, **`normalCommentaryDirect`**, budgets LLM **`fast/normal/thinkerLlmBudget`**, **`generationLimits`**, **`latencyTargetsSec`**) | `ChatResponseModeContentService`, `ChatResponseModeService`, `ChatOperationalLlmSynthesisBriefDirectService` |
