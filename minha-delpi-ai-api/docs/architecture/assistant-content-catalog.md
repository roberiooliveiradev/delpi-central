...
| `presentation_profiles.json` | Perfis declarativos de apresentação (defaultView, viewOrder, stackPlan, flags, **visualBuilders**, **entitySets**, **entityPathHints**, **pathEntityFallbacks**, …) | `ChatPresentationProfileService` → `commentary_profile_key()` → **`ChatDataInsightService`** / **`ChatOperationalDataCommentaryService`**; audit: `scripts/audit_presentation_coverage.py --check-commentary-profiles` |
