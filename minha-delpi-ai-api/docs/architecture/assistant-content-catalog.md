...
| `response_modes.json` | Modos Rápida/Normal/Pensador (rótulos, aliases, avisos de `responseModeEffect`, **`fastCommentaryDirect`**, **`normalCommentaryDirect`**, **`fastLlmBudget`**, **`normalLlmBudget`**) | `ChatResponseModeContentService`, `ChatResponseModeService`, `ChatOperationalLlmSynthesisBriefDirectService` |
