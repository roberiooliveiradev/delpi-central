...
| `drawing_stamp.json` | Rótulos de carimbo, regiões, exclusões cliente/BOM, clarificações, gate nativo (Onda 14), **padrões de cota/decape** (`decapeNote`, `decapeMachineSide`, `cotaDecapeLength`, `intermediateSegment`) | `ChatDrawingStampExtractionService`, `ChatDrawingDimensionsExtractionService`, `ChatDrawingPatternsService` |
| `document_vision.json` | Padrões de intent (OCR e descrição visual); prompts VLM; rótulos de contexto; modos de ativação da skill; **`pdfExtraction`** (fusão multi-fonte, tabelas por anotação, perfis `generic`/`drawing_delpi`, OCR regional condicional) | `ChatDocumentVisionContentService`, `ChatPdfDocumentExtractionService`, `ChatAttachmentDocumentIntentService`, `ChatDocumentVisionSkillService`, `ChatDocumentVisionContextService` |
