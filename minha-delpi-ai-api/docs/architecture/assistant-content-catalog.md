...
| `presentation_profiles.json` | Perfis declarativos de apresentação (defaultView, viewOrder, stackPlan, flags, **visualBuilders**, **chartPolicy**, **visualBundle**, **`textEmbedTreeOutline`**, **`textEmbedTablesInMarkdown`**, **`textEmbedChartsInMarkdown`**) | `ChatPresentationProfileService` → … **`ChatPresentationTreeMarkdownService`**, **`ChatPresentationTableMarkdownService`**, **`ChatPresentationChartMarkdownService`** |
