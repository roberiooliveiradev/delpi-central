...
| `operational_route_registry.json` | Catálogo declarativo DOCIE (`dispatchOrder`, `fallbackPolicies`, ~78 rotas, `match.productionOperationalKind` PB15, predicados, intent binding) | `OperationalRouteRegistryService`, `ExternalActionOperationalRouteSelectionService`, `ExternalActionSqlFallbackPolicyService`, `OperationalRouteRegistryLintService` |
| `product_query_intent.json` | Marcadores de intenção operacional; **`routePredicates`**, **`playbookPredicates`**, **`subIntentPredicates`**, **`intentDetectPipeline`**, **`intentRefinementPipeline`**, **`singleScopeIntentMap`** (DOCIE Fase 11–15); vocabulário em seções (`directives`, `purchases`, `factoryStatus`, …) | `ChatProductQueryIntentService`, `ChatProductQueryIntentDetectionService`, `ChatProductRoutePredicateService`, `OperationalRouteMatcherService` |
