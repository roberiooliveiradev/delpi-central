...
| `drawing_stamp.json` | Rótulos de carimbo, regiões, **`layoutAnalysis`** (XY-Cut), exclusões cliente/BOM, clarificações, gate nativo (Onda 14), padrões de cota/decape, **`bomColumnHeaders`** / **`bomRowRefinement`** (15.8 backlog) | `ChatDrawingStampExtractionService`, `ChatDrawingPageLayoutAnalysisService`, `ChatDrawingDimensionsExtractionService`, `ChatDrawingPatternsService`, `ChatDocumentVisionBomColumnService` (planejado) |
| `document_vision.json` | Intent OCR, VLM, `pdfExtraction` (fusão, perfis `generic`/`drawing_delpi`); **`pdfExtraction.tableStructure`** (15.8, genérico) | `ChatDocumentVisionContentService`, `ChatPdfDocumentExtractionService`, `ChatDocumentVisionSkillService` |
