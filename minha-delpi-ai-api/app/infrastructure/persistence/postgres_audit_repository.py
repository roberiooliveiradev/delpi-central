...
class PostgresAuditRepository(AuditRepositoryPort):
    ...
    def get_sql_advanced_summary(self, *, hours: int = 168) -> dict:
        ...
        for model in models:
            metadata = model.audit_metadata if isinstance(model.audit_metadata, dict) else {}
            snapshot = metadata.get("sqlAdvancedMetrics")
        ...
        return ChatAdvancedSqlMetricsService.aggregate_snapshots(
            entries,
            hours=safe_hours,
            since_iso=since.isoformat(),
        )
    ...
    def get_session_memory_summary(self, *, hours: int = 168) -> dict:
        ...
        for model in models:
            metadata = model.audit_metadata if isinstance(model.audit_metadata, dict) else {}
            snapshot = metadata.get("sessionMemoryAdminMetrics")
        ...
        return ChatSessionMemoryAdminMetricsService.aggregate_snapshots(
            entries,
            hours=safe_hours,
            since_iso=since.isoformat(),
        )
    ...
    def get_typing_correction_summary(self, *, hours: int = 168) -> dict:
        ...
        event_models = (
            AiAuditLogModel.query.filter(
                AiAuditLogModel.created_at >= since,
                AiAuditLogModel.action == "chat.typing_correction.event",
            )
            .order_by(AiAuditLogModel.created_at.desc())
            .limit(3000)
            .all()
        ...
        acceptance_entries: list[dict] = []

        for model in acceptance_models:
            metadata = model.audit_metadata if isinstance(model.audit_metadata, dict) else {}
            snapshot = metadata.get("typingCorrectionMetrics")
        ...
        return ChatTypingCorrectionAdminMetricsService.aggregate(
