...
class ChatCanvasAmbiguityService:
    ...
    def is_deictic_canvas_request(cls, message: str) -> bool:
        ...
        return any(term in normalized for term in cls._DEICTIC_TERMS)
    ...
    def build_clarification_answer(cls, *, previous_messages: list[Any] | None) -> str | None:
        ...
        options = ", ".join(candidates[:-1]) + f" ou {candidates[-1]}"

        return (
            f"Você quer colocar na lousa {options}?\n\n"
            "Responda com uma frase curta (ex.: «a última resposta», «a tabela» ou «o resumo do arquivo»)."
        )
