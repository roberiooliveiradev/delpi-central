"""Orquestração do turno SSE — extraído de ``StreamChatMessageUseCase``."""
...
class ChatStreamTurnExecutionDeps:
    ...
    session_title_service: ChatStreamSessionTitleService
    stream_prepare: ChatStreamTurnPrepareService
...
class ChatStreamTurnExecutionService:
    @classmethod
    def iter_turn(
        cls,
    ...
    def _iter_turn_body(
        ...
        *,
        ...
        user_persist = ChatStreamUserMessageService.persist(
            ...
            turn_support=deps.turn_support,
            request=request,
            message=message,
            session=session,
            user_id=user_id,
        ...
        should_generate_session_title = deps.session_title_service.should_generate(
            session,
            previous_messages,
            resend_from_message_id=resend_from_message_id,
        )
        ...
        if should_generate_session_title:
            deps.session_title_service.apply_fallback_rename(
                deps.chat_repository,
                session_id=session_id,
                user_id=user_id,
                message=message,
            )
            yield {
                ...
                "title": deps.session_title_service.fallback_from_message(message),
            }
        ...
        prepare_state = deps.stream_prepare.start_worker(
            request=request,
            message=message,
            session=session,
            user_id=user_id,
        ...
        if memory_answer:
            yield from cls._yield_memory_limited_turn(
                deps=deps,
        ...
        if persist_before_playback:
            assistant_placeholder = deps.chat_repository.create_message(
                session_id=session_id,
                role="assistant",
                content="",
            ...
            deps.chat_repository.set_active_leaf_message_id(
                session_id=session_id,
                user_id=user_id,
        ...
        if direct_answer:
            ...
            for token in deps.llm_gateway.stream(llm_messages):
        ...
        yield from cls._yield_completion_events(
            deps=deps,
            completion=completion,
            sources=sources,
            persist_before_playback=persist_before_playback,
            should_generate_session_title=should_generate_session_title,
            session_id=session_id,
            user_id=user_id,
            message=message,
        )
    ...
    @classmethod
    def _yield_memory_limited_turn(
        cls,
        *,
        deps: ChatStreamTurnExecutionDeps,
        prepare_state,
        ...
        assistant_message = deps.chat_repository.create_message(
            session_id=session_id,
            role="assistant",
    ...
    def _yield_completion_events(
        ...
        *,
        ...
        if should_generate_session_title:
            deps.session_title_service.schedule_llm_refine(
                chat_repository=deps.chat_repository,
