...
import { fetchConsumptionAnalysisSummary } from "../api/safetyStockApi";
...
export function useConsumptionAnalysisSummary(
  appliedParams: ConsumptionAnalysisQueryParams | null,
) {
  const [data, setData] = useState<ConsumptionAnalysisSummaryData | null>(null);
  ...
  useEffect(() => {
    ...
    async function run() {
      try {
        ...
        const result = await fetchConsumptionAnalysisSummary(params, {
