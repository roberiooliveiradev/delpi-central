...
export function useConsumptionAnalysisItems(
  appliedParams: ConsumptionAnalysisQueryParams | null,
  page: number,
  pageSize: number,
) {
  const [data, setData] = useState<ConsumptionAnalysisItemsData | null>(null);
  ...
  useEffect(() => {
    ...
    async function run() {
      try {
        ...
        const result = await fetchConsumptionAnalysisItems(params, page, pageSize, {
