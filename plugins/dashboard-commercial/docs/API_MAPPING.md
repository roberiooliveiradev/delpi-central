...
| `getSalesOrderOtdPanel` | GET | `/sales-order-otd/panel` | filtros + `status`, paginação e ordenação |
| `getSalesOrderOtdSeries` | GET | `/sales-order-otd/series` | `granularity`, período, `branch`, `customer_segment` |
| `getSalesOrderOtdLineDetail` | GET | `/sales-order-otd/lines/{branch}/{order_number}/{line_item}` | período + segmento |
| `getSalesOrderOtd` | GET | `/sales-order-otd` | KPI escalar (home) |
