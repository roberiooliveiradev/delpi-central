...
const BADGE_CLASS_BY_TYPE: Record<string, string> = {
  PA: "dc-rich-tree__badge--pa",
  PI: "dc-rich-tree__badge--pi",
  MP: "dc-rich-tree__badge--mp",
};
...
function TreeChevronIcon({ expanded }: { expanded: boolean }) {
  return (
    <svg
      className="dc-rich-tree__toggle-icon"
      width="12"
      height="12"
      viewBox="0 0 12 12"
      aria-hidden="true"
...
function RichTreeNodeRow({
  ...
  const badgeHint =
    ...
    COMMERCIAL_HELP_TOOLTIPS.detail.structureType;

  return (
    <li className="dc-rich-tree__item">
      <div
        className="dc-rich-tree__row"
        style={{ paddingLeft: `${depth * 1.1 + 0.35}rem` }}
      >
        {hasChildren ? (
          <button
            type="button"
            className={`dc-rich-tree__toggle ${
              expanded
                ? "dc-rich-tree__toggle--expanded"
                : "dc-rich-tree__toggle--collapsed"
            }`}
          ...
          <span className="dc-rich-tree__toggle-spacer" aria-hidden="true" />
        )}

        <div className="dc-rich-tree__content">
          <div className="dc-rich-tree__primary">
            <span
              className="dc-rich-tree__label"
              title={`${COMMERCIAL_HELP_TOOLTIPS.detail.structureTreeCode}: ${node.label}`}
            >
              {node.label}
            </span>
            {node.badge ? (
              <span
                className={`dc-rich-tree__badge ${badgeClass}`.trim()}
                title={badgeHint}
              >
                {node.badge}
              </span>
            ...
            {node.metaCaption ? (
              <span
                className="dc-rich-tree__meta"
                title={COMMERCIAL_HELP_TOOLTIPS.detail.structureTreeQuantity}
              >
                {node.metaCaption}
              </span>
            ) : null}
          ...
          {node.subtitle ? (
            <div
              className="dc-rich-tree__subtitle"
              title={COMMERCIAL_HELP_TOOLTIPS.detail.structureTreeDescription}
            >
              {node.subtitle}
            </div>
          ) : null}
      ...
      {hasChildren && expanded ? (
        <ul className="dc-rich-tree__children">
...
export function RichTree({
  ...
  return (
    <div className="dc-rich-tree">
      <div className="dc-rich-tree__scroll" style={{ maxHeight }}>
        <ul className="dc-rich-tree__list">
          <RichTreeNodeRow
            node={root}
            depth={0}
            defaultExpanded
      ...
      <div className="dc-rich-tree__footer">{footerLabel(nodeCount)}</div>
    </div>
  );
}
