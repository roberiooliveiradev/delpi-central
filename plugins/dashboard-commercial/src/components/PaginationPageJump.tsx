...
function resolveJumpErrorMessage(
  ...
  switch (reason) {
    case "empty":
      return COMMERCIAL_HELP_TOOLTIPS.pagination.jumpEmpty;
    case "invalid":
      return COMMERCIAL_HELP_TOOLTIPS.pagination.jumpInvalid;
    case "below_min":
      return COMMERCIAL_HELP_TOOLTIPS.pagination.jumpBelowMin;
    case "above_max":
...
export function PaginationPageJump({
  ...
  return (
    <div className="dc-pagination__jump">
      <label className="dc-pagination__jump-field" htmlFor={inputId}>
        <span className="dc-pagination__jump-label">Ir para</span>
        <input
          id={inputId}
          className={
            error
              ? "dc-pagination__jump-input dc-pagination__jump-input--invalid"
              : "dc-pagination__jump-input"
          }
          type="text"
          inputMode="numeric"
          autoComplete="off"
      ...
      />
      {error ? (
        <span id={errorId} className="dc-pagination__jump-error" role="alert">
          {error}
        </span>
      ) : null}
    </div>
