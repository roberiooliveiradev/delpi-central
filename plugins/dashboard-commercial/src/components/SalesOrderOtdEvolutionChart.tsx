...
type SalesOrderOtdEvolutionChartProps = {
  data: SalesOrderOtdSeriesPoint[];
  branch?: string;
  loading?: boolean;
...
export function SalesOrderOtdEvolutionChart({
  data,
  branch,
  loading = false,
  onDrillDown,
}: SalesOrderOtdEvolutionChartProps) {
  ...
  const handleClick: React.ComponentProps<typeof LineChart>["onClick"] = (state) => {
    if (!state) return;
      ...
      >
        ...
        {showFilial01 ? (
          <Line
            type="monotone"
            dataKey="otdFilial01"
            name={COMMERCIAL_OTD_SERIES_LABELS.filial01}
        ...
        {showFilial02 ? (
          <Line
            type="monotone"
            dataKey="otdFilial02"
            name={COMMERCIAL_OTD_SERIES_LABELS.filial02}
