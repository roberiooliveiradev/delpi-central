...
export function MultiSelectField({
  ...
  const rootClass = ["ds-field", "ds-field--multi-select", className]
    .filter(Boolean)
    .join(" ");

  return (
    <div className={`ds-filter-box ${rootClass}`.trim()} ref={wrapperRef}>
      <FieldLabel label={label} hint={labelHint} className="ds-field__label" />
      <div className={`ds-multi-select${open ? " ds-multi-select--open" : ""}`}>
        ...
        >
          <span className="ds-multi-select__trigger-label">{triggerLabel}</span>
          <ChevronDown size={16} aria-hidden="true" />
        </button>

        {open ? (
          <div className="ds-multi-select__panel">
            {searchable ? (
              <input
                type="search"
                className="ds-multi-select__search"
                placeholder="Buscar…"
                value={query}
            ...
            <div className="ds-multi-select__actions">
              <button
                type="button"
                className="ds-ghost-btn ds-btn--sm"
            ...
            <ul id={listId} className="ds-multi-select__list" role="listbox" aria-multiselectable="true">
              {filteredOptions.length === 0 ? (
                <li className="ds-multi-select__empty">Nenhuma opção encontrada.</li>
              ) : (
                filteredOptions.map((option) => (
                  <li key={option.value}>
                    <label className="ds-multi-select__option" title={option.label}>
                      <input
                        type="checkbox"
