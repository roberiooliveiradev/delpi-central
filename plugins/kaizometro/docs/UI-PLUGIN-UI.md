...
O plugin **kaizometro** concluiu a migração de primitivos duplicados para [`@delpi/plugin-ui`](../../plugin-ui/README.md). Textos PT-BR permanecem em `src/content/helpTooltips.ts`; o pacote compartilhado cuida de layout, acessibilidade e factories BEM com prefixo **`kz`**.
...
3. **CSS no plugin** — classes `kz-*` em `src/index.css`; tokens `--delpi-ui-*` no root `.dashboard-kaizometro`.
...
- `components/evidence/index.ts` — dropzone, fila, preview, edição inline (`KaizenEvidenceEditForm`)
...
cd plugins/kaizometro && npm run build
