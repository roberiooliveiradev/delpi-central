/**
 * Auditor de cobertura de help — MFE Manutenção.
 * Detecta DmNative*Field sem hint e colunas DataTable sem headerHint.
 */
import { readdirSync, readFileSync, statSync } from "node:fs";
import { join, relative } from "node:path";

export const FIELD_TAG_RE = /DmNative(?:Text|Select|TextArea)Field\b/;
...
export const SKIP_FILES = new Set([
  "app/maintenanceUi.ts",
  "components/dmFormFields.ts",
...
/**
 * @param {string} rootDir absolute path to plugins/maintenance/src
 * @returns {string[]}
 */
...
export function findFieldGaps(relativeFile, source) {
  ...
  const gaps = [];
  const re = /<(DmNative(?:Text|Select|TextArea)Field)\b/g;
  let match;
