...
export function useLoadingProgress(active: boolean, progress: RequestProgress): number {
  return useMemo(
    () => getLoadingPercent(active, progress),
    [active, progress.completed, progress.total],
  );
}

function beginSingleRequestProgress(onProgress: (progress: RequestProgress) => void): void {
  onProgress({ completed: 0, total: 1 });
}

function finishSingleRequestProgress(
