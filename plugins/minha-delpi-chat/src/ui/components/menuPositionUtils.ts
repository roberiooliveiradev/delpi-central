...
function resolveComposerPopoverLayout(
  ...
  let maxHeight = openAbove
    ? Math.min(naturalHeight, spaceAbove - gap)
    : Math.min(naturalHeight, spaceBelow - gap);
  ...
  if (menuTop + maxHeight > viewport.height - margin) {
    maxHeight = Math.max(options.minHeight, viewport.height - margin - menuTop);
  }

  return { left: menuLeft, top: menuTop, maxHeight };
}
