...
- Baixar e **enviar** anexo (colar imagem / clipe / arrastar) estão publicados (H12 live em produção: `GLPI_LEGACY_*` + Document). Preview no editor usa `blob:`; resize no canto da imagem. `@` no compositor, aprovar/reabrir/satisfação e fila técnica ficam fora até HLAPI/decisão.
