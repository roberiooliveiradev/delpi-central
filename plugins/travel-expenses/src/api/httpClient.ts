...
const DELPI_CALLER_APP = "travel-expenses";
...
async function parseError(response: Response, bodyText?: string): Promise<string> {
  ...
  if (looksLikeHtml(text, response.headers.get("content-type"))) {
    return "API de despesas de viagem indisponível. Verifique o serviço travel-expenses-api e o gateway.";
  }
  try {
  ...
  if (response.status === 502 || response.status === 503) {
    return "API de despesas de viagem indisponível. Tente novamente em instantes.";
  }
...
export async function httpForm<T>(url: string, form: FormData, options: RequestOptions = {}): Promise<T> {
  const response = await fetch(url, {
    method: "POST",
    headers: authHeaders(),
    body: form,
