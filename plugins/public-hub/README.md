...
- **Detalhe da operação:** tocar no card ativo ou em **Detalhes** na tabela abre `OperationDetailPage` — dados da operação à esquerda, visualização à direita (abas **Desenho** = PDF do PA e **3D** = modelo do `product_code` da OP), botão **Voltar para a fila** e **Tela cheia** reaproveitando o `DrawingViewer`. Só mostra o que a fila publicada já traz (quantidades, programação, entrega, apontamentos, `has_3d_model`); nenhuma consulta nova ao TOTVS. Sem PA, sem arquivo PDF ou sem GLB daquele produto, a coluna direita explica o motivo. Abaixo de 1000px as colunas empilham.
- **Desempenho do posto:** no hero do posto, dois tiles clicáveis (eficiência do turno com cor por faixa e horas paradas hoje) e o botão **Ver desempenho** no topo abrem `WorkCenterPerformancePage` com KPIs e gráficos do kit (`ConfigurableSeriesChart`: área com rótulos nos pontos, barras horizontais de motivos com todos os nomes visíveis via `categoryLabelOverflow: wrap`, barras por dia). Fonte: `GET /apps/production-control-api/public/machine-load/{token}/performance?branch=&workCenter=&days=`, com refresh próprio de 5 min (a fila continua em 15s). Enquanto carrega — ou se a api-delpi estiver indisponível — os tiles mostram `—` e a fila segue normal. Turno sem apontamento aparece vazio, nunca como 0%. Import estático nomeado do remote `@delpi/plugin-ui` (o lazy dinâmico do Index quebrava com React #306 no MF).
