...
- **Layout:** cabeçalho em duas faixas (*Minha produção* + hero do centro de trabalho), card **Agora nesta bancada** (OP em produção ou a primeira da fila) e tabela **Próximas operações** com busca por OP/PA. A largura do palco é liberada por app (`.pub-content--fullpage:has(.pcp-pub)`). Em telas estreitas o hero e o card ativo empilham; a tabela mantém scroll horizontal intencional.
- **Desenho do PA:** botão **Ver desenho / 3D** no card ativo quando há `pa_product_code` e/ou modelo 3D. O hub chama `GET /apps/production-control-api/public/machine-load/{token}/drawings/{pa}/pdf?branch=…`; o BFF só entrega se o PA estiver na fila publicada e obtém o PDF da **api-delpi** (`GET /products/{code}/drawing/pdf`). O FILESERVER não é montado no PCP. Pré-requisito de infra: mount `DRAWING_PDF_*` na api-delpi ([infra/README-ambiente.md](../../infra/README-ambiente.md) § Biblioteca PDF de desenhos).
- **Modelo 3D do produto da OP:** quando o item da fila traz `has_3d_model`, o card ativo oferece **Ver desenho / 3D** (ou **Ver 3D** se não houver PA). O GLB vem de `GET /apps/production-control-api/public/machine-load/{token}/models/{product_code}/glb?branch=…` — chaveado pelo **produto da operação** (`product_code`, PI ou PA), não pelo PA do desenho. O BFF só lê o volume se aquele código aparece como produto de alguma OP visível na fila publicada. O anexo é feito no Portal PCP (`/apps/production-control/product-models`).
- **Detalhe da operação:** tocar no card ativo ou em **Detalhes** na tabela abre `OperationDetailPage` — dados da operação à esquerda, visualização à direita (abas **Desenho** = PDF do PA e **3D** = modelo do `product_code` da OP), botão **Voltar para a fila** e **Tela cheia** reaproveitando o `DrawingViewer`. O modal **Apontamentos** carrega a tabela (data, quantidade, operador) via `GET .../operations/appointments`. Sem PA, sem arquivo PDF ou sem GLB daquele produto, a coluna direita explica o motivo. Abaixo de 1000px as colunas empilham.
- **Desempenho do posto:** no hero do posto, tiles clicáveis (eficiência do turno com cor por faixa, **peças produzidas no turno** e horas paradas no turno) e o botão **Ver desempenho** no topo abrem `WorkCenterPerformancePage` com KPIs, gráficos do kit e tabela de apontamentos do turno (PA, OP, operador, tempos e eficiência). Fonte: `GET /apps/production-control-api/public/machine-load/{token}/performance?branch=&workCenter=&days=`, com refresh próprio de 5 min (a fila continua em 15s). Enquanto carrega — ou se a api-delpi estiver indisponível — os tiles mostram `—` e a fila segue normal. Turno sem apontamento aparece vazio, nunca como 0%. Import estático nomeado do remote `@delpi/plugin-ui` (o lazy dinâmico do Index quebrava com React #306 no MF).
- **Navegação sem sub-rota:** `resolveRoute` só aceita três segmentos após `/p/`, então detalhe e desempenho são estado interno espelhado na query (`?view=operation&op=&operation=`, `?view=performance`) via `replaceState` — recarregar o tablet volta à mesma tela.
