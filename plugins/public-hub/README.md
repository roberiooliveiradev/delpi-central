...
- **Detalhe da operação:** tocar no cartão abre `OperationDetailPage` — dados da operação à esquerda, preview do desenho à direita, botão **Voltar para a fila** e **Tela cheia** reaproveitando o `DrawingViewer`. Só mostra o que a fila publicada já traz (quantidades, programação, entrega, apontamentos); nenhuma consulta nova ao TOTVS. Sem PA ou sem arquivo, a coluna direita explica o motivo em vez de exibir um iframe quebrado. Abaixo de 1000px as colunas empilham.
