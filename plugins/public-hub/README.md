...
- **Modelo 3D do produto da OP:** quando o item da fila traz `has_3d_model`, o cartão oferece **Ver desenho / 3D** (ou **Ver 3D** se não houver PA). O GLB vem de `GET /apps/production-control-api/public/machine-load/{token}/models/{product_code}/glb?branch=…` — chaveado pelo **produto da operação** (`product_code`, PI ou PA), não pelo PA do desenho. O BFF só lê o volume se aquele código aparece como produto de alguma OP visível na fila publicada. O anexo é feito no Portal PCP (`/apps/production-control/product-models`).
- **Detalhe da operação:** tocar no cartão abre `OperationDetailPage` — dados da operação à esquerda, visualização à direita (abas **Desenho** = PDF do PA e **3D** = modelo do `product_code` da OP), botão **Voltar para a fila** e **Tela cheia** reaproveitando o `DrawingViewer`. Só mostra o que a fila publicada já traz (quantidades, programação, entrega, apontamentos, `has_3d_model`); nenhuma consulta nova ao TOTVS. Sem PA, sem arquivo PDF ou sem GLB daquele produto, a coluna direita explica o motivo. Abaixo de 1000px as colunas empilham.
- **Desempenho do posto:** dois chips na barra (eficiência do turno corrente, com cor por faixa, e horas paradas hoje) e o botão **Ver desempenho**, que abre `WorkCenterPerformancePage` com KPIs, gráfico de eficiência por dia, barras de horas paradas por motivo e por dia, e a lista de apontamentos do turno **sem identificação nominal**. Fonte: `GET /apps/production-control-api/public/machine-load/{token}/performance?branch=&workCenter=&days=`, com refresh próprio de 5 min (a fila continua em 15s). Enquanto carrega — ou se a api-delpi estiver indisponível — os chips mostram `—` e a fila segue normal. Turno sem apontamento aparece vazio, nunca como 0%. Gráficos via `ConfigurableSeriesChart` do remote `@delpi/plugin-ui` (import estático nomeado — o lazy dinâmico do Index quebrava com React #306 no MF).
