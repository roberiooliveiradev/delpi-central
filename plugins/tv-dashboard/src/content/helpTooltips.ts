export const TV_DASHBOARD_HELP_TOOLTIPS = {
  ribbon: {
    ...
    paragraphSpacing:
      "Estilo tipográfico nomeado, entrelinhas e espaçamento antes/depois do parágrafo.",
    paragraphSpacingBefore: "Espaço acima do parágrafo, em pixels.",
    ...
    kpiAppearance:
      "Tema Claro/Escuro e tom semântico (padrão, positivo, negativo, atenção) do card KPI. Na faixa: «Alterar estilos» (mesmo painel do pincel flutuante).",
    tableStyleOptions:
  ...
  data: {
    ...
    dateRangePreset:
      "Escolha um período relativo (recalculado a cada atualização da TV, no fuso America/Sao_Paulo) ou Personalizado para Data início/fim. «Até hoje» cobre do início do período até a data corrente inclusive; datas fixas escondidas na UI não congelam o fim. «Esta semana / Este mês / Este trimestre / Este ano» (sem «até hoje») cobrem o calendário civil completo (fim pode ser futuro). «Dia anterior» e «Este mês (até ontem)» usam o último dia útil (segunda → sexta). «Ano passado» = 01/01–31/12 do ano civil anterior; «Este mês ano passado» = mês civil completo do mesmo número no ano anterior; «Mesmo período ano passado» = 01/01 até o mesmo dia/mês no ano anterior (espelho de «Este ano até hoje»). Em rotas com intervalo fechado (ex. PPM), o período é obrigatório — sem default automático.",
    filterUnsetHere:
      "Não definido aqui",
    ...
    textDataBinding:
      "Projeta um campo da fonte ou do filtro efetivo do slide (ex.: filter.start_date / filter.end_date) no bloco. Prefixo/sufixo e formato (incl. data) aplicam-se ao valor dinâmico; alterar filtros atualiza o preview. Sem valor, o template permanece e o trecho dinâmico mostra «—». Campo inválido é rejeitado na gravação — não é convertido silenciosamente.",
    textDataColorRules:
  ...
  dataPrepare: {
    ...
    tabTransform: "Renomear, filtrar, substituir, ordenar, manter/remover linhas, tipo e preencher.",
    tabAddColumn: "Coluna personalizada, etapa fx, agrupar, pivot e unpivot.",
    tabCombine: "Mesclar esta consulta com outra fonte do mesmo slide.",
    refresh: "Recalcula a prévia no servidor com as etapas até o ponto selecionado.",
    selectColumns: "Mantém só as colunas informadas (remove as demais).",
    promoteHeaders: "Usa a primeira linha de dados como nomes de coluna.",
    routePreset: "Aplica as etapas sugeridas no overlay da rota api-delpi (quando existir).",
    actionRename: "Renomeia a coluna ativa (clique no cabeçalho do grid).",
    actionFilter: "Filtra linhas pela coluna ativa e condição.",
    actionReplace: "Substitui texto na coluna ativa.",
    actionSort: "Ordena as linhas pela coluna ativa.",
    actionKeepRows: "Mantém N linhas do topo ou da base.",
    actionRemoveRows: "Remove N linhas do topo ou da base.",
    actionChangeType: "Converte a coluna ativa para número ou texto.",
    actionFillDown: "Preenche células vazias com o valor da linha acima.",
    actionCustomColumn: "Cria coluna com expressão DSL (if, concat, operadores…).",
    actionFx: "Insere nova etapa editável na barra fx (como o botão fx do Power Query).",
    actionGroupBy: "Agrupa por chaves e agrega uma coluna.",
    actionPivot: "Pivot: valores da coluna viram novas colunas.",
    actionUnpivot: "Unpivot: colunas viram pares atributo/valor.",
    actionMerge: "Left join com outra consulta do slide.",
    formulaBar:
      "Fórmula da etapa selecionada. ✓ aplica, ✕ ou Escape descarta. Não grava ao sair do campo.",
    formulaApply: "Aplica a fórmula e recalcula a prévia no servidor.",
    formulaDiscard: "Descarta a edição e volta ao texto da etapa (ou cancela nova coluna).",
    queries: "Fontes data_source do slide. Clique para selecionar; clique de novo para desselecionar.",
    queryItem: "Consulta ligada a uma rota api-delpi. Botão direito: atualizar, limpar etapas…",
    previewBanner: "A grade mostra o resultado até a etapa selecionada (cálculo no backend).",
    grid: "Clique no cabeçalho para selecionar/desselecionar a coluna. Botão direito: ações da coluna.",
    columnHeader: "Coluna da prévia. Ligada a série do gráfico quando houver vínculo no slide.",
    settings: "Nome da consulta, coluna ativa e lista de etapas aplicadas.",
    steps: "Clique numa etapa para ver a prévia até ali; clique de novo volta à Fonte. Botão direito: editar/excluir.",
    stepFonte: "Dados crus da API, sem transformações. Clique para desselecionar etapas.",
    stepItem: "Etapa da Query. Lápis foca a barra fx; botão direito abre o menu.",
    stepEdit: "Edita a etapa selecionada na barra de fórmulas.",
    stepMoveUp: "Move a etapa uma posição para cima na sequência.",
    stepMoveDown: "Move a etapa uma posição para baixo na sequência.",
    stepDelete: "Remove a etapa da Query (a prévia recalcula).",
    newFxStep: "Cria uma nova etapa addColumn via barra fx.",
    footerCancel: "Fecha o modal sem forçar refresh dos visuais do palco.",
    footerApply: "Fecha e atualiza a prévia da fonte no slide.",
  },
  dataPrepareContextMenu: {
    menu: "Ações do objeto na preparação de dados",
    refresh: "Atualizar prévia",
    clearSteps: "Limpar todas as etapas",
    copyRoute: "Copiar operationId",
    editFx: "Editar na barra fx",
  ...
  contextMenu: {
    ...
    alignCenterV: "Alinhar ao meio",
    alignBottom: "Alinhar parte inferior",
    distributeH: "Distribuir horizontalmente",
    distributeV: "Distribuir verticalmente",
    alignSlideLeft: "Alinhar à esquerda do slide",
    alignSlideCenterH: "Centralizar horizontalmente no slide",
    alignSlideRight: "Alinhar à direita do slide",
    alignSlideTop: "Alinhar à parte superior do slide",
    alignSlideCenterV: "Centralizar verticalmente no slide",
    alignSlideBottom: "Alinhar à parte inferior do slide",
    rotate: "Girar",
    rotateCw: "Girar 90° para a direita",
    rotateCcw: "Girar 90° para a esquerda",
  ...
  filmstripContextMenu: {
    ...
    duplicateMany: "Duplicar {count} telas",
    rename: "Renomear tela",
    hide: "Ocultar tela",
    hideMany: "Ocultar {count} telas",
    show: "Exibir tela",
    showMany: "Exibir {count} telas",
    delete: "Excluir tela",
    deleteMany: "Excluir {count} telas",
  },
  homeContextMenu: {
