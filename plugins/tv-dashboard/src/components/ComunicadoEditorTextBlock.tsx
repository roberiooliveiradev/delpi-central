...
export function ComunicadoEditorTextBlock({ block, fontScale = 1, className = "", isSelected }: Props) {
  const { updateBlockContent } = useComunicadoEditor();
  ...
  const blockClass = [
    ...
    isSelected ? "td-composer__text-block--editing" : "td-composer__text-block--readonly",
    className,
  ]
    .filter(Boolean)
    .join(" ");
