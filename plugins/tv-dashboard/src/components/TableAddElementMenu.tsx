...
import type { ComunicadoTableOptions, TableElementId } from "@delpi/tv-dashboard-presentation";
...
type Props = {
  ...
  onApplyChoice: (choiceId: TableAddElementChoiceId) => void;
  onMoreOptions: (elementId: TableElementId) => void;
  className?: string;
};

/**
 * Menu cascata PPT para tabela — espelho de ChartAddElementMenu / KpiAddElementMenu.
 * Compartilhado ribbon + float `+`.
 */
export function TableAddElementMenu({
  options,
  onApplyChoice,
  onMoreOptions,
  className,
  ...
  const [openId, setOpenId] = useState<TableElementId | null>(null);
            ...
            {open ? (
              ...
              >
                {root.choices.map((choice) => {
                  ...
                  const active = isTableAddElementChoiceActive(choice.id, options);
                  return (
                    <li key={choice.id}>
                      <button
                        type="button"
