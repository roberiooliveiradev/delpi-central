...
- **Modo de reprodução:** `playbackMode` na programação — **apresentação** (auto-advance por duração) ou **reunião** (avanço manual). Resolução efetiva: `?mode=presentation|meeting` → override de sessão (toolbar) → default da playlist. Ortogonal a `?presenter=1`.
  ...
  - **Caneta / laser (reunião):** toolbar com Caneta, Laser e Limpar; traços e ponteiro sincronizam ao vivo (`meeting_ink_stroke`, `meeting_laser`, `meeting_ink_clear`). **Caneta por slide:** rabisco permanece ao navegar (até Limpar ou F5); overlay exibe só a tela atual. **Laser:** paint local/remoto via DOM + rAF (sem re-render do deck); rede throttled ~90 ms — fluido no kiosk Adeus Pendrive/TV. Estado **só em memória** — F5 limpa; late join **não** restaura ink. Modo apresentação não mostra as ferramentas.
  - **Checklist live (reunião):** 2 abas no mesmo link — caneta em qualquer slide; voltar ao slide anterior mantém rabisco; laser contínuo na TV Adeus (sem trava/salto); Limpar só o slide atual; **F5: overlay vazio**; apresentação sem ferramentas e com auto-advance; sync de slide (`playback_cursor`) ok. Rebuild: `./infra/scripts/up-dev-sequential.sh --fase core --build tv-dashboard-api tv-dashboard public-hub`.
