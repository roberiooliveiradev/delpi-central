...
import type { MinuteDetail } from "../api/cecApi";
import { MinuteDocumentView } from "./MinuteDocumentView";

vi.mock("../api/cecApi", async (importOriginal) => {
  const original = await importOriginal<typeof import("../api/cecApi")>();
...
const detail: MinuteDetail = {
  ...
  participants: [
    {
      user_id: "u1",
      display_name: "Ano Comitê de Ética",
...
describe("MinuteDocumentView", () => {
  it("renderiza leitura formal com participantes, conteúdo e assinaturas", () => {
    ...
    expect(screen.getByText("Comitê de Ética e Conduta")).toBeTruthy();
    expect(screen.getByText("Integridade, respeito e responsabilidade")).toBeTruthy();
    expect(screen.getByText("Ata de reunião do Comitê de Ética")).toBeTruthy();
    expect(screen.getAllByText(/Ano Comitê de Ética/).length).toBeGreaterThan(0);
