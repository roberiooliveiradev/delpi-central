...
const DELPI_CALLER_APP = "comite-etica-conduta";
...
async function parseError(response: Response, bodyText?: string): Promise<string> {
  ...
  if (looksLikeHtml(text, response.headers.get("content-type"))) {
    return "API Comitê de Ética indisponível (resposta HTML no lugar de JSON). Verifique se o serviço comite-etica-conduta-api e o gateway estão no ar.";
  }
  try {
  ...
  if (response.status === 502 || response.status === 503) {
    return "API Comitê de Ética indisponível. Tente novamente em instantes.";
  }
...
async function parseJson<T>(response: Response): Promise<T> {
  ...
  try {
    ...
    throw new Error(
      "Resposta inválida da API Comitê de Ética (não é JSON). Verifique o serviço comite-etica-conduta-api e o gateway.",
    );
  }
}
