...
describe("MinuteEditorPage composição Comitê de Ética", () => {
  it("pré-carrega membros ativos na nova ata", async () => {
    render(<MinuteEditorPage unitCode="01" />);

    await waitFor(() => {
      expect(api.listComiteEticaMembers).toHaveBeenCalledWith("01", {
        activeOn: expect.any(String),
      });
    });
    expect(screen.getAllByText("Ana Presidente").length).toBeGreaterThan(0);
    expect(screen.getAllByText("Presidente").length).toBeGreaterThan(0);
    expect(
      screen.getByRole("button", { name: /Recarregar composição Comitê de Ética/ }),
    ).toBeTruthy();
  });
  ...
  it("não sobrescreve participantes ao editar ata existente", async () => {
    ...
    expect(api.listComiteEticaMembers).not.toHaveBeenCalled();
    expect(screen.queryByText("Ana Presidente")).toBeNull();
    expect(
      screen.queryByRole("button", { name: /Recarregar composição Comitê de Ética/ }),
    ).toBeNull();
  });
