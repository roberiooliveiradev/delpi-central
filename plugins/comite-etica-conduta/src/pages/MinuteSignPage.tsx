...
export function MinuteSignPage({ unitCode, minuteId }: Props) {
  ...
  async function submitSignature(signatureBlob: Blob) {
    ...
    try {
      ...
      navigateCec(`/apps/comite-etica-conduta/minutes/${minuteId}`);
    } catch (err) {
  ...
  return (
    <div className="cec-page-stack comite-etica-conduta-sign-page">
      <ComiteEticaPageHeader
        nav={
          <BackLink
            variant="prominent"
            onClick={() => navigateCec(`/apps/comite-etica-conduta/minutes/${minuteId}`)}
          >
            Voltar para a ata
          </BackLink>
        }
      ...
      <div ref={signSectionRef} className="cec-sign-anchor" />
      <ComiteEticaSectionCard title="Confirmar assinatura">
        <div className="cec-field">
          <FieldLabel label="Nome do signatário" htmlFor="comite-etica-conduta-signer-name" />
          <NativeTextControl id="comite-etica-conduta-signer-name" value={name} onChange={setName} />
        </div>
        <NativeCheckboxControl
          className="cec-check"
          checked={accepted}
        ...
        {hasSavedSignature && savedPreviewUrl ? (
          <div className="cec-signature-preview">
            <p>
              ...
              <ActionButton
                variant="link"
                onClick={() => navigateCec("/apps/comite-etica-conduta/minha-assinatura")}
              >
                (gerenciar)
              </ActionButton>
            </p>
      ...
      <ComiteEticaSectionCard title="Recusar assinatura">
        <NativeTextAreaControl
          value={refuseReason}
