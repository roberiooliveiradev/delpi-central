...
import { fetchComiteEticaAccess, type ComiteEticaAccessPayload } from "../api/cecApi";
import { buildComiteEticaAccessFromPermissions } from "../security/buildCecAccess";
...
function normalizeAccessPayload(data: ComiteEticaAccessPayload): ComiteEticaAccess {
  return {
...
export function useCecAccess(
  ...
  const localAccess = useMemo(
    () =>
      canResolveLocally
        ? buildComiteEticaAccessFromPermissions(permissionCodes, isSuperadmin)
        : null,
    [canResolveLocally, isSuperadmin, permissionCodes],
  );

  const [remoteAccess, setRemoteAccess] = useState<ComiteEticaAccess | null>(null);
