...
export function ChartGranularityToggle({
  ...
  return (
    <div className="dq-segment-toggle" role="group" aria-label="Agrupamento do gráfico">
      {CHART_GRANULARITY_OPTIONS.map((option) => (
        <button
          key={option.value}
          id={`${idPrefix}-granularity-${option.value}`}
          type="button"
          className={`dq-segment-toggle__btn${
            value === option.value ? " dq-segment-toggle__btn--active" : ""
          }`}
