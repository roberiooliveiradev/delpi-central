...
| `getScrapCostPct` | GET | `/quality/scrap-cost-pct` | `branch`, `date_start`, `date_end` — numerador SBC exclui produto de terceiro (`B1_TPMAT = 2`) |
...
**Campo no frontend:** `list_kaizen[].daily_savings`, `list_kaizen[].annual_savings` (`daily_savings × 253 dias úteis`) e `total_savings` (soma ponderada por dias ativos no período filtrado).
