export const helpTooltips = {
  ...
  lineFeeder:
    "Matéria-prima que precisa estar nas bancadas até o horário escolhido. Intermediários e acabados da estrutura não entram — o alimentador cuida do que vem do almoxarifado. A necessidade vem das ordens da programação com início até esse horário; do que é preciso, desconta-se o que já está no ponto de uso, e o restante é o que falta entregar. Quando duas bancadas pedem o mesmo material, o estoque é dividido por horário: quem começa antes é atendido primeiro. «Em risco» é o que não cabe no saldo do almoxarifado. No tablet a lista abre em cards para caber na tela sem scroll lateral (dá para voltar à tabela pela barra). A bolinha indica bloqueio de inventário no almoxarifado (vermelho = bloqueado, verde = livre). O local de retirada vem do cadastro do produto na filial. Clique na linha para abrir o detalhe: visão geral com o saldo real do almoxarifado, bancadas do corte e transferências recentes entre armazéns. A lista de coleta agrupa por produto (soma entre bancadas) e ordena pelo código; cada item vai de a coletar para separado e entregue, e nada disso escreve no TOTVS.",
  lineFeederPickPlan:
