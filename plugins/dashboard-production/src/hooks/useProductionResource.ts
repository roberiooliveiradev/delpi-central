...
type UseProductionResourceOptions = {
  enabled?: boolean;
  cacheTtlMs?: number;
  cacheKey?: string;
};
...
type UseProductionResourceResult<T> = {
  data: T | null;
  loading: boolean;
  error: string | null;
  reload: () => void;
...
export function useProductionResource<T>(
  fetcher: (signal: AbortSignal) => Promise<T>,
  deps: readonly unknown[],
  options: UseProductionResourceOptions = {}
): UseProductionResourceResult<T> {
  ...
  useEffect(() => {
    ...
    async function run() {
      try {
        ...
        if (!cached) {
          const message = formatProductionApiError(err);
          if (message) {
            setError(message);
          }
        }
