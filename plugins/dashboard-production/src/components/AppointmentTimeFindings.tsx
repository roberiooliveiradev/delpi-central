...
import type { AppointmentTimeFinding } from "../types/production";
...
export function AppointmentTimeFindings({ findings }: AppointmentTimeFindingsProps) {
  if (!findings.length) {
    return (
      <p className="dp-time-findings__ok" role="status">
  ...
  return (
    <ul className="dp-time-findings" aria-label="Alertas da análise de tempos">
      {findings.map((finding) => (
        <li
          key={finding.code}
          className={`dp-time-findings__item dp-time-findings__item--${finding.severity}`}
        >
