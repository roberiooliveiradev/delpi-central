...
export function MultiSelectField({
  ...
  return (
    <div className={`dp-filter-box ${rootClass}`.trim()} ref={wrapperRef}>
      <FieldLabel label={label} hint={labelHint} className="dp-field__label" />
      <div className={`dp-multi-select${open ? " dp-multi-select--open" : ""}`}>
        <button
          type="button"
          className="dp-multi-select__trigger"
          aria-expanded={open}
        ...
        >
          <span className="dp-multi-select__trigger-label">{triggerLabel}</span>
          <ChevronDown size={16} aria-hidden="true" />
        </button>

        {open ? (
          <div className="dp-multi-select__panel">
            {searchable ? (
              <input
                type="search"
                className="dp-multi-select__search"
                placeholder="Buscar…"
                value={query}
            ...
            <ul id={listId} className="dp-multi-select__list" role="listbox" aria-multiselectable="true">
              {filteredOptions.length === 0 ? (
                <li className="dp-multi-select__empty">Nenhuma opção encontrada.</li>
              ) : (
                filteredOptions.map((option) => (
                  <li key={option.value}>
                    <label className="dp-multi-select__option" title={option.label}>
                      <input
                        type="checkbox"
