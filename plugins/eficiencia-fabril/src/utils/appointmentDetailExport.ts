import type {
  ProductionOeeAppointmentDetail,
  ProductionOeeAppointmentDetailData,
} from "../types/productionOeeDetail";
...
function formatAppointmentStatusLabel(
  appointment: Pick<ProductionOeeAppointmentDetail, "oee_pct">
): string {
  if (isProductionEfficiencyOutlier(appointment.oee_pct)) return "Verificar";
  if (isProductionEfficiencyLow(appointment.oee_pct)) return "Eficiência baixa";
  return "OK";
}
...
export function buildAppointmentDetailExportDocument(
  ...
  const appointmentRows: [string, string][] = [
    ["Status", formatAppointmentStatusLabel(appointment)],
