...
| `editor-chrome.css` | `EditorChrome` + `EditorChromeNotice(s)` + `EditorHistoryActions` — `.delpi-ui-editor-chrome*` / `-notice*` / `-history*` |
...
| `MentionComposer` | **Contenteditable** markdown (não textarea cru): `value`/`onChange` em markdown; `formatToggle` (B/I/S/lista/código/quote/link + Align L/C/R/J = `text-align` no **`<p>` da seleção**, após `ensureComposerParagraphFlow` — órfãos/`<br>` viram parágrafo; a imagem não herda o align do bloco anterior); `EmojiInsertMenu`; `submitOnEnter` (Shift+Enter = **novo parágrafo**, não `<br>`); faixa `replyTo` + `onCancelReply`; thumbs na pílula + `__document-tray` para não-imagem; imagens **inline Word** (`insertComposerInlineImageAtCaret` via **Range**, nunca `insertHTML`; clipboard `uniqueClipboardImageFiles` files XOR items — **uma** captura; paste misto Word: `materializeClipboardHtmlInlineImages` mantém texto+formatação e troca `<img>` por pending na ordem; screenshot só-imagem continua só files; ZWSP; `img` `display:inline` + `vertical-align:baseline`; × remove; `resolveAttachmentImageSrc` no edit); anexar/enviar. |
