...
Shell canônico do kit para controles densos (tipo de gráfico, overlays YoY), preferências em `localStorage` (mesmo padrão de `usePersistedViewLayout`) e plot Recharts multi-tipo. Tendência linear por série e ponderação de período parcial vivem no inspector **Séries**, não no overlay **Opções**.
...
| Overlay options | `ChartOverlayOptionsPopover` + `buildCompareYearsOverlayOptions` (YoY; ponderação de período parcial fica no inspector Séries) |
