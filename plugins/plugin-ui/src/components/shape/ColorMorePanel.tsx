...
export type ColorMorePanelProps = {
  value?: string;
...
/**
 * Conteúdo de «Mais cores» — abas Padrão / Personalizar (sem ModalShell).
 * Usado pelo popover aninhado e pelo ColorDialog legado do catálogo.
 */
export function ColorMorePanel({
  value,
  onConfirm,
  onCancel,
  labels,
  ...
  return (
    <div className={["delpi-ui-shape-dialog__layout", className].filter(Boolean).join(" ")}>
      ...
      <aside className="delpi-ui-shape-dialog__aside">
        <button
          ...
          onClick={() => onConfirm(colorToCss(draft))}
        >
          {L.ok}
        </button>
        <button type="button" className="delpi-ui-shape-btn delpi-ui-shape-btn--ghost" onClick={onCancel}>
          {L.cancel}
        </button>
