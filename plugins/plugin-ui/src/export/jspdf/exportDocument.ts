import { stripExportFilenameExtension } from "../matrixAdapter";
import { sanitizeFilename, sanitizeSheetName } from "../primitives";

function exportBasename(filename: string): string {
  return sanitizeFilename(stripExportFilenameExtension(filename));
}
...
export async function exportTableExcel(table: ExportTable, filename: string): Promise<void> {
  ...
  XLSX.writeFile(workbook, `${exportBasename(filename)}.xlsx`);
}
...
export async function exportTablePdf(table: ExportTable, filename: string): Promise<void> {
  ...
  doc.save(`${exportBasename(filename)}.pdf`);
}
