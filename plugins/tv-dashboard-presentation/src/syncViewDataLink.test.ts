...
describe("syncViewDataLink", () => {
  it("materializa kpiProjection multi e amplia frame default", () => {
    const patch = buildViewDataLinkPatch({
      ...
      currentFrame: { x: 8, y: 32, w: 20, h: 15 },
    });
    ...
    expect(patch.frame?.w).toBeGreaterThan(20);
    expect(patch.frame?.h).toBeGreaterThanOrEqual(14);
  });
  ...
  it("suggestViewFrameSize cresce para 7 KPIs", () => {
    const next = suggestViewFrameSize("kpi_view", 7, { x: 8, y: 32, w: 20, h: 15 });
