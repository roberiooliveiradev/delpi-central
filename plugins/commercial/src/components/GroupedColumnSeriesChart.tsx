...
export type GroupedColumnBarSpec = {
  ...
  name: string;
  fill: string;
  /** When true, OLS trend line is computed from this series (requires showTrend). */
  trendSource?: boolean;
};
...
export type GroupedColumnSeriesChartProps = {
  ...
  height?: number;
  showTrend?: boolean;
  /** Override default trend field prefix `_trend`. One field per trendSource bar. */
...
const TREND_STROKE = "var(--cm-fg-muted, #64748b)";
...
/**
 * Recharts grouped columns + optional linear trend line(s) for billing/ROL series.
 */
              ...
              <Line
                ...
                name={
                  trendSources.length > 1
                    ? `${CUSTOMER_BILLING_CONTENT.trendLineSeriesName} (${source.name})`
                    : CUSTOMER_BILLING_CONTENT.trendLineSeriesName
                }
                stroke={TREND_STROKE}
                strokeWidth={2}
                strokeDasharray="4 4"
