...
import { getPropostaComercial, openPropostaComercialPdf } from "../../api/propostasComerciaisApi";
import {
...
export function PropostaDetailPage({ basePath, propostaId }: PropostaDetailPageProps) {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setError(null);
    void getPropostaComercial(propostaId, controller.signal)
      .then((result) => {
  ...
  const itemColumns: DataTableColumn<PropostaComercialItem>[] = [
  ...
  function buildPdfOverrides(): PropostaComercialPdfExportOverrides | undefined {
    if (!data) return undefined;
    const overrides: PropostaComercialPdfExportOverrides = {};
    const obs = pdfObservacoes.trim();
    if (obs && obs !== (data.observacoes || "").trim()) {
      overrides.observacoes = obs;
    }
    const contato: NonNullable<PropostaComercialPdfExportOverrides["contato"]> = {};
  ...
  async function handleExportPdf() {
    try {
      setPdfLoading(true);
      setPdfError(null);
      await openPropostaComercialPdf(propostaId, buildPdfOverrides());
    } catch (err) {
  ...
  return (
    <section className="cm-page-stack">
      <header className="cm-page-header-row">
        <CommercialTitleWithHelp
          title={cabecalho?.numero_ov || PROPOSTAS_CONTENT.detail.title}
          hint={
            data
        ...
        <div className="cm-nav-row">
          {canExportPropostas ? (
            ...
            >
              ...
              {pdfLoading ? "Gerando…" : PROPOSTAS_CONTENT.detail.exportPdf}
            </ActionButton>
          ) : null}
      ...
      {!loading && data ? (
        <>
          ...
          <SectionCard
            title={PROPOSTAS_CONTENT.detail.items}
            classNames={cmSectionCardClassNames}
            labels={cmSectionLabels}
          >
            <DataTable
