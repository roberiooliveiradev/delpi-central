...
export function resolveLinkedDateFilters(input: {
  ...
  const dateEnd =
    input.dateEnd && ISO_DATE_RE.test(input.dateEnd) ? input.dateEnd : input.defaultDateEnd;
  const competence =
    input.competence && isValidCompetence(input.competence) ? input.competence : "";
...
export function applyDateStartChange(dateStart: string, dateEnd: string): LinkedDateFilters {
  return {
    dateStart,
    dateEnd,
    competence: dateRangeToCompetence(dateStart, dateEnd),
...
export function applyDateEndChange(dateStart: string, dateEnd: string): LinkedDateFilters {
  return {
    dateStart,
    dateEnd,
    competence: dateRangeToCompetence(dateStart, dateEnd),
