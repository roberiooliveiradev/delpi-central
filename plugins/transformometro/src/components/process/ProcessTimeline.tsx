...
import {
  ...
  filterProcessTimelineEntries,
  PROCESSO_TIMELINE_FILTER_OPTIONS,
  type ProcessTimelineCategory,
  type ProcessTimelineEntry,
  type ProcessTimelineFilter,
...
const CATEGORY_ICONS = {
  ...
  vinculo: Link2,
} as const satisfies Record<ProcessTimelineCategory, typeof FileText>;

function TimelineItem({ entry }: { entry: ProcessTimelineEntry }) {
...
export function ProcessTimeline({ entries, loading = false }: Props) {
  const [filter, setFilter] = useState<ProcessTimelineFilter>("all");
  const timeline = useMemo(() => buildProcessTimeline(entries), [entries]);
  const visibleEntries = useMemo(
    () => filterProcessTimelineEntries(timeline, filter),
    [timeline, filter]
  );
