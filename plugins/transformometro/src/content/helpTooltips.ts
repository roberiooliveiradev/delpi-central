...
export const TM_HELP_TOOLTIPS = {
  dashboard: {
    ...
    kpis: {
      economiaLiquida:
        "Economia líquida do recorte: economia bruta menos os custos (investimentos e rateio de recursos) no período.",
      economiaBruta:
        "Ganho bruto estimado no período, antes de descontar custos.",
      solucoes:
        "Quantidade de soluções implementadas que compõem o recorte.",
      horas:
        "Total de horas de trabalho economizadas no período no recorte.",
      roi:
  ...
  instancias: {
    escopo:
      "Cada instância pertence a uma unidade (ou a todas as ativas, consolidada) e amarra um ou mais setores. As revisões ficam na instância.",
    todasUnidades:
      "Cria/mantém uma instância única consolidada, válida para todas as unidades ativas — em vez de uma por unidade.",
    unidades:
