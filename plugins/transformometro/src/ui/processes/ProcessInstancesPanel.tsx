...
import { ImprovementPhaseBadge } from "../../components/improvement/ImprovementPhaseBadge";
import { ImprovementFolderBrowser } from "./ImprovementFolderBrowser";
import {
...
type Props = {
  ...
  options: OptionsData;
  processScope?: ProcessoEscopoState | null;
  busy?: boolean;
...
export function ProcessInstancesPanel({
  instancias,
  selectedInstanciaId,
  options,
  processScope = null,
  ...
  const processoTemEscopo = hasProcessoEscopo(processScope);
  ...
  useEffect(() => {
    ...
    if (!hasProcessoEscopo(processScope) || !processScope) return;
    setUsarEscopoProcesso(true);
    setTodasFiliais(processScope.todas_filiais_ativas);
    setFilialIds(processScope.todas_filiais_ativas ? [] : [...processScope.filial_ids]);
    setSetorIds([...processScope.setor_ids]);
    if (!processScope.todas_filiais_ativas && processScope.filial_ids[0]) {
      setFilialId(processScope.filial_ids[0]);
    }
  ...
  function applyProcessoEscopo() {
    ...
    const resolved = resolveCreateInstanciaEscopo(options, processScope, true);
  ...
  function openCreateForm() {
    resetForm();
    const resolved = resolveCreateInstanciaEscopo(options, processScope, processoTemEscopo);
    setFilialId(resolved.filialId);
    setFilialIds(resolved.filial_ids);
    setSetorIds(resolved.setor_ids);
    setTodasFiliais(resolved.todas_filiais_ativas);
  ...
  const columns = useMemo<DataTableColumn<ProcessoInstancia>[]>(
    () => [
      ...
      },
      {
        ...
        render: (row) => <ImprovementPhaseBadge fase={row.fase_melhoria} />,
      },
      {
        key: "prioridade",
        header: "Prioridade",
