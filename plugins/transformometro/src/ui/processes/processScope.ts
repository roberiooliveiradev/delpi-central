...
export function processScopeFromEntity(processo: Processo): ProcessoEscopoState {
  return {
...
export function resolveCreateInstanciaEscopo(
  options: OptionsData,
  processScope: ProcessoEscopoState | null | undefined,
  preferProcesso: boolean
): ProcessoEscopoState & { filialId: string } {
  if (preferProcesso && hasProcessoEscopo(processScope) && processScope) {
    const firstFilial =
      processScope.filial_ids[0] ?? options.filiais[0]?.id ?? "01";
    return {
      todas_filiais_ativas: processScope.todas_filiais_ativas,
      filial_ids: processScope.todas_filiais_ativas ? [] : [...processScope.filial_ids],
      setor_ids: [...processScope.setor_ids],
...
export function processScopePayload(escopo: ProcessoEscopoState): Partial<Processo> {
  return {
