 ...
 * All authorized units (or empty MultiSelect = Todas) is NOT active by itself.
...
export function hasActivePurchaseOrdersFilters(
  ...
  const defaultUnits = allowedUnits.length === 0 || sameUnitSet(query.branches, allowedUnits);
  return Boolean(
    !defaultUnits ||
