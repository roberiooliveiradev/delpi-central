...
class ReposicaoRepository(PluginBaseRepository):
    ...
    def media_golpes(
        ...
        *,
        ...
        return self.media_golpes_map(filial=filial).get(key, 0.0)

    def media_golpes_map(self, *, filial: str) -> dict[tuple[str, str], float]:
        rows = self.fetch_all(
            f"""
            SELECT codigo_ferramenta, codigo_peca, COALESCE(AVG(golpes), 0) AS media
            FROM {VW_REPOSICOES_PREVENTIVA}
            WHERE filial = %s
            GROUP BY codigo_ferramenta, codigo_peca
            """,
            (filial,),
        )
        return {
            (str(row["codigo_ferramenta"]), str(row["codigo_peca"])): float(row.get("media") or 0)
            for row in rows
        }

    def golpes_history_map(self, *, filial: str) -> dict[tuple[str, str], list[int]]:
        rows = self.fetch_all(
            f"""
        ...
        for row in rows:
            ...
            result.setdefault(key, []).append(int(row["golpes"]))
        return result
