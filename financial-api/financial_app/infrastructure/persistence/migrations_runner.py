...
from financial_app.infrastructure.persistence.plugins_postgres_connection import (
    FIN_SCHEMA_NAME,
    get_connection,
)
...
class MigrationError(RuntimeError):
    """Erro de execução de migrations do Portal Financeiro."""


def ensure_migrations_table(conn: Any) -> None:
    with conn.cursor() as cur:
        cur.execute(f'CREATE SCHEMA IF NOT EXISTS "{FIN_SCHEMA_NAME}";')
        cur.execute(
            f"""
            CREATE TABLE IF NOT EXISTS "{FIN_SCHEMA_NAME}".schema_migrations (
...
def get_applied_migrations(conn: Any) -> dict[str, dict[str, Any]]:
    with conn.cursor() as cur:
        cur.execute(
            ...
            FROM "{FIN_SCHEMA_NAME}".schema_migrations
...
def apply_migration(conn: Any, path: Path) -> None:
    ...
    try:
        with conn.cursor() as cur:
            ...
            cur.execute(
                f"""
                INSERT INTO "{FIN_SCHEMA_NAME}".schema_migrations (version, name, checksum)
                VALUES (%s, %s, %s)
                """,
...
def run_migrations() -> None:
    ...
    with get_connection() as conn:
        ...
        if not pending:
            print("[financial] Nenhuma migration pendente.")
            return

        print("[financial] Executando migrations...")
        for path in pending:
            apply_migration(conn, path)
        print("[financial] Migrations aplicadas com sucesso.")
...
def show_status() -> None:
    ...
    with get_connection() as conn:
        ...
        print("Status das migrations do Portal Financeiro:")
        for path in files:
            version, name = parse_version_and_name(path)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Runner de migrations Postgres do financial-api.",
    )
