...
class DeviceOtaWakeService:
    """Best-effort push wake after OTA authorize. Never fails the OTA target."""
    ...
    def wake_target(self, target: dict[str, Any]) -> None:
        ...
        if str(target.get("status") or "") != "authorized":
            return
        if target.get("wake_attempted_at") is not None:
            return

        claimed = self._jobs.claim_wake_attempt(UUID(str(target_id)))
        if claimed is None:
            return
