...
def register_device_drivers() -> None:
    ...
    registry.register_implementation(Esp32c3CounterDriver())
    registry.register_implementation(Esp32CounterDriver())
    registry.register_implementation(Esp8266GaugeDriver())
    _registered = True
    logger.info(
        "Device drivers registered: esp8266_counter_v1, esp32c3_counter_v1, "
        "esp32_counter_v1, esp8266_gauge_v1"
    )
