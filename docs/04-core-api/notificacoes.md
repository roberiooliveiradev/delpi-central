...
| `category` | `system`, `welcome`, `birthday`, `company_event`, `announcement`, `access`, `custom`, `controle_mp` |
...
Admin e integrações: `POST /admin/notifications`, `POST /integrations/notifications` (vários `emails` na mesma requisição), templates em `/admin/notifications/templates`, auditoria em `GET /admin/notifications/dispatches` (filtros), detalhe/revogação em `GET/DELETE /admin/notifications/dispatches/:id`, revogação em lote em `POST .../dispatches/bulk-delete`, edição de agendados em `PUT .../dispatches/:id` (somente `pending` com `scheduled_at` futuro), processamento em `POST .../dispatches/process-pending` — ver [roadmap](../12-roadmap-e-evolucao/notificacoes-ricas.md).
