...
IDD, classificação, metas e realizado estratégico da Visão geral têm owner **strategic-indicators-api**. A supplies-api lê o escopo ativo em uma chamada `api-delpi` `GET /dashboard/department-indicators` (`department_id=supplies`, `branch` só quando há uma unidade). O MFE não chama o SI e não mostra breakdown do consolidado.
