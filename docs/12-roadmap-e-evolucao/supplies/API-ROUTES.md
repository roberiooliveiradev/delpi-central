...
| Estoque | GET | `/inventory/stock-balances/summary` | `supplies.access` + filtro de dados `01`/`02` | api-delpi `GET /supplies/stock-balances/summary` (`only_positive=false`; Todas=`branch=01&branch=02`) | **IMPLEMENTADO** (E10.S1–S3; Help/smoke = S4) |
| Estoque | GET | `/inventory/stock-balances/items` | `supplies.access` + filtro de dados `01`/`02` | api-delpi `GET /supplies/stock-balances/items` (`only_positive=false`; UM/`null`/`QATU` passthrough) | **IMPLEMENTADO** (E10.S1–S3; Help/smoke = S4) |
