...
### Apontamentos Portal (set/2026) — gaps fechados / CRM
...
| P-LEAD-PLACE-FAT | Lead time colocação→faturamento | **Backlog CRM** | Depende de P-PLACE-DATE | apontamentos #5 |
| P-CYCLE-DATES | Datas do ciclo (confirmação→produção→embarque…) | **Backlog CRM** | CRM-DEL-003 / timeline | apontamentos #4 |
...
|----|------|--------|--------|
| P-OFF-SLA | Etapas / SLA / área | **Parcial** | UI **Administração → SLAs** + CRUD `GET/POST/PATCH/DELETE` `/settings/sla-policies` (soft deactivate). Valores de prazo ainda dependem de homologação de negócio (lista pode ficar vazia). Consumo operacional (banner/alertas em Oportunidades/etapas) **fora** desta entrega. |
