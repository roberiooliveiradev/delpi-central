...
| Fonte | api-delpi `get_financial_rol`, `get_*_rol_target_pct`, `get_commercial_rol_series`; BFF commercial-api `/analytics/*`; SQL: `api-delpi/app/infrastructure/persistence/totvs/financial_repositories/financial_repository.py` · helper de expressão: `CommercialRolReturnSql` (`commercial_rol_return_sql.py`) |
...
| **Natureza na Minha Carteira (P-BRUTO-LIQ)** | Toggle `nature=gross\|net` (default **gross**). **net** = mesma fórmula ROL acima. **gross** = `F2_VALBRUT` (série/Fat.12m/tendência) ou `gross_revenue` do envelope ROL-by-customer (share/ranking). Contrato ecoa `nature` + `supportedNatures`. MFE: `?billingNature=`. |
