...
| GET | `/analytics/portfolio-billing-share` | `bff_get_analytics_portfolio_billing_share` | Carteira comparativos | `analytics.view` **ou** `accounts.team.view` **ou** `seller-portfolios.manage` | `portfolio_billing_share` | `scalar` | KPI-PORTFOLIO-SHARE; `{ portfolioRol, companyRol, sharePct, startDate, endDate, branch, nature }`; denominador = ROL empresa sem `customer_codes`; smoke `tests/test_portfolio_billing_share.py` |
| GET | `/analytics/portfolio-billing-ranking` | `bff_get_analytics_portfolio_billing_ranking` | Carteira comparativos | `accounts.view` (cliente) · `group_by=seller` exige team/manage | `portfolio_billing_ranking` | `list` | Delta % ROL vs período −1a; `{ groupBy, items[], startDate, endDate, prior*, nature }`; MFE Minha Carteira + Excel; smoke `tests/test_portfolio_billing_ranking.py` |
