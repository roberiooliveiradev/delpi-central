...
| GET | `/analytics/portfolio-billing-share` | `bff_get_analytics_portfolio_billing_share` | Carteira comparativos | `analytics.view` **ou** `accounts.team.view` **ou** `seller-portfolios.manage` | `portfolio_billing_share` | `scalar` | KPI-PORTFOLIO-SHARE; query `nature=gross\|net` (default gross → `gross_revenue`; net → `rol`); payload `{ …, nature, kpiNature, billingNature }`; smoke `tests/test_portfolio_billing_share.py` |
| GET | `/analytics/portfolio-billing-ranking` | `bff_get_analytics_portfolio_billing_ranking` | Carteira comparativos | `accounts.view` (cliente) · `group_by=seller` exige team/manage | `portfolio_billing_ranking` | `list` | Delta % vs período −1a; query `nature=gross\|net` (mesma regra share); MFE Minha Carteira + Excel; smoke `tests/test_portfolio_billing_ranking.py` |
...
| POST | `/customers/billing-series` | `list_commercial_customer_billing_series` | F2b | accounts.view | `billing_series` | `scalar` | Body `nature=gross\|net` (default gross = `F2_VALBRUT`; net = ROL por bucket); eco `nature`/`supportedNatures`; `filter_pairs` antes do gateway |
| POST | `/customers/enrichment` | (BFF enrichment) | F2b | accounts.view | — | — | Body `nature` + `window_days`; Fat.12m / tendência / recent-prior na mesma natureza |
