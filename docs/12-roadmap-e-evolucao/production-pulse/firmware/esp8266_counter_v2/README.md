...
| `FIRMWARE_VERSION` | `esp8266_counter_v2.0.0` |
| Contrato `/api/*` | Idêntico ao v1 (driver `esp8266_counter_v1`) |
| UI `/` | Badge **V2**, accent verde, exibe versão |

## Lab swap v1 ↔ v2

1. Compile/publique o `.bin` do v1 (`esp8266_counter_v1.3.0`) e do v2 (`esp8266_counter_v2.0.0`) no catálogo OTA (mesma `firmwareKey` / `driverKey`).
