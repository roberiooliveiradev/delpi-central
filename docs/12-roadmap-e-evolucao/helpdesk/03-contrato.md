...
`GET /tickets/{id}` inclui `description`, `created_at` (instante de abertura), `status` (rótulo), `status_id` (id ITIL, aditivo), `requester_display_name` (primeiro membro de `team` com papel `requester`; se faltar, `user_recipient`), `requester_mine`, `assigned_display_name` (primeiro `assigned` do `team`), `timeline[]` com `id`, `kind` (`followup`), `content`, `created_at`, `author_display_name`, `mine`, e `attachments[]` com `document_id`, `filename` e `mime`. `description` e `timeline[].content` são **texto puro**: o tradutor remove o HTML que o GLPI grava. Evolução aditiva do corpo (`description_html` / `content_html`, sanitizado) está só em [`12-conteudo-da-mensagem.md`](./12-conteudo-da-mensagem.md) e **não** está autorizada. O nome de pessoa é só rótulo. `mine` / `requester_mine` vêm do id do usuário na sessão HLAPI (`GET /session` → `user_id`) ou do e-mail do JWT contra o e-mail do autor; nome nunca identifica pessoa. O nome visível usa o rótulo mais completo entre `firstname`+`realname` e `display_name`. A lista de anexos traz só arquivos já ligados àquele chamado. Lista vazia é `[]`. Acompanhamento privado e tarefa não entram em `timeline`. Os campos novos são aditivos.
