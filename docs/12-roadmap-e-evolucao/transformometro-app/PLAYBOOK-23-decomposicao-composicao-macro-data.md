...
| Diagrama | Playbook 19 S7: mesmo conceito deste playbook (âncora + `diagrama/composed` + visão vigente primeiro). |
...
1. ~~**Diagrama = mesmo conceito deste playbook (PB19 S7)**~~ — **entregue**.  
2. Baseline da melhoria B partir do **composto na data de início** dela (foto do «agora» global) — WBS e diagrama.  
...
5. Motor unificado WBS+diagrama (só se duplicação excessiva; preferir serviços irmãos).
...
- [x] **PB19 S7** — mesmo conceito no diagrama (âncora + composed + visão vigente primeiro)
