...
TEST_ADAPTER = DeterministicTestAdapter (TEST_ONLY; not production)
REAL_PROVIDER_ADAPTER = NONE
...
#### C3-T4 Structured Understanding (APPROVED)
...
OUTPUT_SCHEMA_MODEL = ACCEPT (capability-specific parser; SchemaRegistry = NOT REQUIRED)
...
VERDICT = ACCEPT_WITH_RESIDUAL
OWNER = delia-api capability projection / OpenAPI catalog foundation
CAPABILITY_PROJECTION = PROJECTION_ONLY
...
#### C3-T6 Expertise / Knowledge Governance + Retrieval Contracts (historical; superseded by C3-T6R1)
...
EXPERTISE = ExpertisePack (evidence_refs: tuple[EvidenceRef, ...])
PLAYBOOK = DomainPlaybook / PlaybookStep
