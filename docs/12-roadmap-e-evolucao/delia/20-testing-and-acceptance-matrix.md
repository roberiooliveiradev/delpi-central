...
C2-T3 / C2-PREFINAL-R1: `LOCKED` em `25` para CP-001–CP-012, CP-025, CP-059, CP-061–CP-070, CP-149, CP-156, CP-159 e CP-171 não é evidência de runtime (`LOCKED` ≠ PASS). Host/lifecycle atuais estão cobertos por C2-T1/T2. C2-T4 proved api-delpi contracts for OP/product/operation; MACHINE and POSTO stay `TO_INVENTORY`. C2-T5R3 replaced the superseded modal surface with a non-modal companion dock in source (unit/structural `PASS`). Product Master post-deploy smoke of C2-T5R3R1 passed for the requested scope: dock render, non-modal, no backdrop, current route, dynamic reclamp, center app usable, pointer resize, keyboard resize (`LIVE_DYNAMIC_RECLAMP=PASS`). Route persistence, close/reopen and full-page transition were not part of that smoke. User A/B and portal logout E2E remain `TEST_NOT_RUN`. Portal has no React DOM component harness, so rendered dock integration stays `TEST_NOT_RUN`. Workspace runtime and typed commands remain `NOT_IMPLEMENTED`. The existing Portal embedded postMessage host is inventoried in C2-T6 and is not a DÉLIA bridge. C2-T6R1 keeps CP-061–CP-068 and CP-070 `LOCKED` and CP-069 `TO_INVENTORY`; applicability notes are separate from that status. `C2_SECURITY_BLOCKER=NO`. Portal and Transformômetro security follow-up remains required and does not block C2. CP-012 depende de C3. `PORTAL_LOGOUT_FULL_E2E` e `LIVE_USER_A_USER_B` continuam `TEST_NOT_RUN`.
