...
C2-T3: `LOCKED` em `25` para CP-001–CP-012, CP-025, CP-059, CP-061–CP-070, CP-156, CP-159 e CP-171 não é evidência de runtime. Host/lifecycle atuais estão cobertos por C2-T1/T2. C2-T4 proved api-delpi contracts for OP/product/operation; MACHINE and POSTO stay `TO_INVENTORY`. C2-T5R3 replaced the superseded modal surface with a non-modal companion dock in source (unit/structural `PASS`). Product Master post-deploy evidence: dock renders on `/apps/my-requests`, non-modal, no backdrop, special sidebar launcher removed, normal app entry preserved (`LIVE_COMPANION_DOCK_RENDER=PASS`). C2-T5R3R1 reclamps stored dock width when the workspace narrows (`DYNAMIC_WORKSPACE_RECLAMP=PASS` in unit tests). `LIVE_DYNAMIC_RECLAMP=TEST_NOT_RUN`. Center interaction, route persistence, pointer/keyboard resize, close/reopen and full-page transition remain unproven live. Portal has no React DOM component harness, so rendered dock integration stays `TEST_NOT_RUN`. Workspace runtime, typed commands e bridge de iframe permanecem `NOT_IMPLEMENTED`. CP-012 depende de C3. `PORTAL_LOGOUT_FULL_E2E` e `LIVE_USER_A_USER_B` continuam `TEST_NOT_RUN`.
