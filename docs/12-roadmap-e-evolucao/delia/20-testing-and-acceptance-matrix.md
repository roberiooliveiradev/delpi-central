...
C2-T3: `LOCKED` em `25` para CP-001–CP-012, CP-025, CP-059, CP-061–CP-070, CP-156, CP-159 e CP-171 não é evidência de runtime. Host/lifecycle atuais estão cobertos por C2-T1/T2. C2-T4 proved api-delpi contracts for OP/product/operation; MACHINE and POSTO stay `TO_INVENTORY`. C2-T5 implemented the Portal global launcher/panel with the same federated `delia` / `./App` remote (unit/structural `PASS`; `LIVE_GLOBAL_SURFACE=TEST_NOT_RUN`). C2-T5R1 hardened the mount generation race and dialog focus return/containment at unit level. Portal has no React DOM component harness, so rendered panel integration stays `TEST_NOT_RUN`. Workspace runtime, typed commands e bridge de iframe permanecem `NOT_IMPLEMENTED`. CP-012 depende de C3. `PORTAL_LOGOUT_FULL_E2E` e `LIVE_USER_A_USER_B` continuam `TEST_NOT_RUN`.
