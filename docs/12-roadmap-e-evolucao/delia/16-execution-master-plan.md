...
**Próxima etapa:** `ARCHITECTURE_REVIEW_C3_T6R1` (`C3_AUTHORIZED=YES`; `C3_STARTED=YES`; `C3_EXECUTED=NO`; `C3-T1..T5=APPROVED`; `C3-T6R1=CANDIDATE_FOR_ARCHITECTURE_REVIEW`; `C3_T7_AUTHORIZED=NO`; do not claim C3 complete; `PRODUCTION_READINESS=NOT_PROVEN`; `REAL_DELPI_OPENAPI_COVERAGE=NOT_PROVEN`)
...
→ C3-T6 Expertise / Knowledge Governance + Retrieval Contracts = historical candidate (537cf47664; superseded by C3-T6R1)
→ C3-T6R1 Knowledge Contract Rework = CANDIDATE_FOR_ARCHITECTURE_REVIEW
...
C2-FINAL accepted with residual; C3-T1..T5 APPROVED; C3-T6R1 knowledge contract rework candidate ready for Architecture Review
...
C3-T6R1 corrects Expertise `evidence_refs` to typed `EvidenceRef`, removes retrieval `knowledge_scope` and `include_*` override flags, and hardens normal organizational retrieval to PUBLISHED only (REVOKED/DEPRECATED excluded). No registry/RAG/RetrievalPort/planner/ACT.
