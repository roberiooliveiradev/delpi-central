...
> **Arquivo:** `docs/12-roadmap-e-volucao/kaizometro/ROADMAP.md`  
...
Documentação operacional: [plugins/kaizometro/README.md](../../../plugins/kaizometro/README.md) · [DOCUMENTACAO.md](../../../plugins/kaizometro/docs/DOCUMENTACAO.md)
...
| Docker dev | ✅ | `delpi-kaizometro`, gateway `/apps/kaizometro` |
...
| Homologação automatizada | ⏳ Pendente | Sem `check-kaizometro.sh` nem CI dedicado |
...
- [x] Permissões `kaizometro.view` / `kaizometro.manage` em `api_delpi_permissions.py`
...
## Fase 2 — MFE `kaizometro` ✅

**Entregáveis**

- [x] Vite + Module Federation (`base: /apps/kaizometro/`)
- [x] `kaizometro.manifest.json` (permissões + rota menu)
- [x] Páginas: listagem, novo, editar
- [x] `httpClient` com `X-Delpi-Caller-App: kaizometro`
- [x] API base `/apps/api-delpi/quality/kaizens/records`
- [x] Dockerfile + serviço `kaizometro` em `docker-compose.dev.yml` / `docker-compose.yml`
...
./plugins/kaizometro/scripts/register-manifest.sh
...
**Critério de pronto:** usuário com permissão vê «Kaizômetro» no menu; consegue criar/editar/excluir; importação dry-run OK em staging.
...
- [ ] `scripts/ci/build-kaizometro.sh` — `npm run ci` no plugin
- [ ] `scripts/homologacao/check-kaizometro.sh` — smoke: `remoteEntry.js` + `GET /records` com `TOKEN`
- [ ] `scripts/homologacao/check-kaizometro-api.sh` — E2E: create → get → update → delete → import dry_run
...
- [ ] Link «Cadastrar / editar» no dashboard apontando para `/apps/kaizometro` (quando usuário tem `kaizometro.manage`)
...
- [ ] Actions permitidas no agente de qualidade: `list_kaizen_records`, `get_kaizen_record` (leitura); escrita sob `kaizometro.manage`
...
docker compose -f docker-compose.dev.yml --env-file .env up -d --build \
  postgres-plugins api-delpi kaizometro gateway
...
# Build MFE
cd plugins/kaizometro && npm run ci
```
...
| [plugins/kaizometro/README.md](../../../plugins/kaizometro/README.md) | Guia operacional |
| [plugins/kaizometro/docs/DOCUMENTACAO.md](../../../plugins/kaizometro/docs/DOCUMENTACAO.md) | Contratos HTTP e arquitetura |
...
Em paralelo, planejar **Fase 5** (scripts `check-kaizometro*.sh`) antes do merge amplo em produção.
