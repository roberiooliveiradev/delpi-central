# Playbook — Evolução do Kaizômetro

> **Arquivo:** `docs/12-roadmap-e-volucao/kaizometro/PLAYBOOK-EVOLUCAO-KAIZEN.md`
> **Status:** proposta de design — a implementar (Fases 6+ do [ROADMAP.md](./ROADMAP.md))
> **Produto:** Minha DELPI · módulo Qualidade
> **Escopo:** plugin `kaizometro` + rotas `api-delpi/quality/kaizens/*` + schema `quality`
...
**Objetivo do playbook:** elevar o `kaizometro` ao mesmo patamar de maturidade do PAC, reaproveitando padrões canônicos já existentes no monorepo, sem reinventar arquitetura:
...
- **CSS:** manter raiz `.dashboard-kaizometro` + prefixo `kz-`; adicionar `.kz-section-read/edit`, galeria antes/depois, pipeline. Validar dark mode + mobile (`plugins-visual-design-system.mdc`).
