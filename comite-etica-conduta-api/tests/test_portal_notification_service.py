...
def test_send_builds_core_contract_with_user_ids_and_portal_route():
    service = CecPortalNotificationService(
    ...
    with patch(
        "cec_app.application.services.portal_notification_service.httpx.post",
    ...
    assert body["category"] == "comite_etica_conduta"
    assert body["sourceApp"] == "comite-etica-conduta"
    assert body["action"] == {
        "type": "portal_route",
        "label": "Assinar ata",
        "target": "/apps/comite-etica-conduta/atas/minute-1/sign",
    }
    assert body["metadata"]["event"] == EVENT_SIGN_PENDING
    assert body["metadata"]["dedupeKey"] == "cec:sign_pending:minute-1:user-1"
...
def test_signed_and_refused_deep_link_to_detail():
    ...
    assert targets == [
        "/apps/comite-etica-conduta/atas/m1",
        "/apps/comite-etica-conduta/atas/m1",
    ]
...
def test_disabled_skips_http():
    ...
    with patch(
        ...
        assert service.notify_sign_pending(
            user_id="u1",
            minute_id="m1",
            minute_number="2026/001",
            title="t",
