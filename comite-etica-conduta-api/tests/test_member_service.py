...
def _manager():
    return _user(permissions=["comite-etica-conduta.manage", "comite-etica-conduta.view"])


def _viewer():
    return _user(permissions=["comite-etica-conduta.view", "comite-etica-conduta.view"])
...
def test_list_requires_view_or_manage():
    svc = MemberService()
    user = _user(permissions=["comite-etica-conduta.sign"])
    with pytest.raises(PermissionError):
        svc.list_members(user)
...
def test_create_persists_member():
    svc = MemberService()
    created = _row(role="member", user_id=USER_B, display_name="Bruno")
    with patch.object(svc.repo, "find_active_by_user", return_value=None):
...
def test_end_member_sets_inactive():
    svc = MemberService()
    current = _row()
...
def test_soft_delete_requires_manage_and_keeps_history_flag():
    ...
    with patch.object(svc.repo, "get_member", return_value=current):
        with patch.object(svc.repo, "soft_delete", return_value=deleted) as soft:
            payload = svc.delete_member(_manager(), MEMBER_ID)
...
def test_list_invalid_active_on():
    ...
    with pytest.raises(ValueError, match="vigência"):
        svc.list_members(_viewer(), active_on="16/07/2026")
