...
def test_health_import():
    from cec_app.main import app

    assert "Comitê" in app.title or "Etica" in app.title or "CEC" in app.title
...
def test_html_sanitizer_strips_script():
    cleaned = CecHtmlSanitizer.sanitize(
        '<p>ok</p><script>alert(1)</script><a href="javascript:x">x</a>'
    )
...
def test_html_sanitizer_collapses_word_nbsp_runs():
    ...
    assert "1. Desenvolver ferramenta" in cleaned.replace("<span>", "").replace(
        "</span>", ""
    )
...
def test_html_sanitizer_collapse_nbsp_runs_helper():
    assert CecHtmlSanitizer.collapse_nbsp_runs("a&nbsp;&#160;\u00a0&nbsp;b") == "a b"
    assert CecHtmlSanitizer.collapse_nbsp_runs(None) == ""
...
def test_permission_codes():
    ...
    assert perms.action_permission_code("create") == "comite-etica-conduta.manage"
    assert perms.action_permission_code("sign") == "comite-etica-conduta.sign"


def test_has_unit_action_corporate():
    from types import SimpleNamespace

    viewer = SimpleNamespace(
        permissions=["comite-etica-conduta.view"],
        is_superadmin=False,
    )
    signer = SimpleNamespace(
        permissions=["comite-etica-conduta.sign", "comite-etica-conduta.view"],
        is_superadmin=False,
    )
    admin = SimpleNamespace(permissions=["comite-etica-conduta.manage"], is_superadmin=False)

    assert perms.has_unit_action(viewer, "view", "00")
    assert not perms.has_unit_action(viewer, "create", "00")
    assert perms.has_unit_action(signer, "sign", "00")
    assert perms.has_unit_action(admin, "finalize", "00")
...
def test_unit_codes_for_read():
    ...
    assert perms.unit_codes_for_read(manager) == ["00"]
