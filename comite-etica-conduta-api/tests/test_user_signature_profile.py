...
from cec_app.application.use_cases.user_signature_service import UserSignatureService
...
def test_assert_global_action_requires_sign():
    user = _user(user_id=USER_A, permissions=["comite-etica-conduta.view"])
    with pytest.raises(PermissionError):
        perms.assert_global_action(user, "sign")


def test_assert_global_action_allows_sign():
    user = _user(user_id=USER_A, permissions=["comite-etica-conduta.sign"])
    perms.assert_global_action(user, "sign")
...
def test_user_signature_storage_rejects_non_png(tmp_path):
    ...
    with pytest.raises(CecStorageError):
...
def test_save_and_read_image_isolates_by_token_user(tmp_path):
    ...
    user_a = _user(user_id=USER_A, permissions=["comite-etica-conduta.sign"])
    user_b = _user(user_id=USER_B, permissions=["comite-etica-conduta.sign"])
...
def test_save_image_rejects_non_png():
    ...
    with patch.object(
        svc.storage,
        "save_png",
        side_effect=CecStorageError("Formato de assinatura inválido. Envie uma imagem PNG."),
    ):
