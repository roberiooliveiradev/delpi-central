...
from cec_app.application.services.content_hash_service import ContentHashService
from cec_app.application.services.html_sanitizer import CecHtmlSanitizer
from cec_app.application.services.portal_notification_service import (
    CecPortalNotificationService,
)
from cec_app.application.services.sign_pending_mail_service import (
    CecSignPendingMailService,
)
from cec_app.application.services.storage_services import (
    AttachmentStorageService,
    PdfStorageService,
    SignatureStorageService,
)
from cec_app.domain.services.minute_status_transition_service import (
    MinuteStatusTransitionError,
    MinuteStatusTransitionService,
)
from cec_app.infrastructure.pdf.minute_pdf_renderer import MinutePdfRenderer
...
class MeetingMinutesService:
    def __init__(self) -> None:
        ...
        self.notifications = CecPortalNotificationService()
        self.sign_pending_mail = CecSignPendingMailService()
    ...
    def create(self, user, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        agenda = CecHtmlSanitizer.sanitize(payload.get("agenda_html"))
        body = CecHtmlSanitizer.sanitize(payload.get("body_html"))
        decisions = CecHtmlSanitizer.sanitize(payload.get("decisions_html"))
        pending = CecHtmlSanitizer.sanitize(payload.get("pending_html"))
        observations = CecHtmlSanitizer.sanitize(payload.get("observations_html"))
    ...
    def update(self, user, minute_id: str, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        if any(key in payload for key in content_keys):
            ...
            agenda = CecHtmlSanitizer.sanitize(
                payload.get("agenda_html", current.get("agenda_html"))
            )
            body = CecHtmlSanitizer.sanitize(payload.get("body_html", current.get("body_html")))
            decisions = CecHtmlSanitizer.sanitize(
                payload.get("decisions_html", current.get("decisions_html"))
            )
            pending = CecHtmlSanitizer.sanitize(
                payload.get("pending_html", current.get("pending_html"))
            )
            observations = CecHtmlSanitizer.sanitize(
    ...
    def create_version(self, user, minute_id: str, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        def _content(field: str) -> str:
            ...
            return CecHtmlSanitizer.sanitize(value)
    ...
    def send_for_signature(self, user, minute_id: str) -> dict[str, Any]:
        ...
        minute_title = str(updated.get("title") or "")
        ...
        *,
        ...
        if new_status != minute["status"]:
            ...
            if new_status == "signed":
                ...
                for key in ("responsible_user_id", "created_by_user_id"):
                    uid = str(minute.get(key) or "").strip()
    ...
    def refuse(self, user, minute_id: str, reason: str) -> dict[str, Any]:
        ...
        recipients: set[str] = set()
        for key in ("responsible_user_id", "created_by_user_id"):
            uid = str(updated.get(key) or "").strip()
            if uid and uid != "unknown":
                recipients.add(uid)
        actor_name = (
    ...
    def export_pdf(self, user, minute_id: str) -> tuple[bytes, str]:
        ...
        if minute.get("final_pdf_path"):
            ...
            return raw, f"ata-comite_etica-{minute['minute_number'].replace('/', '-')}.pdf"
        version = self.repo.get_version(minute_id)
        if not version:
            raise LookupError("Versão não encontrada.")
    ...
    def export_filtered_pdfs(self, user, filters: dict[str, Any]) -> tuple[bytes, str]:
        ...
        return buffer.getvalue(), f"atas-comite_etica-{unit_code}-{stamp}.zip"
