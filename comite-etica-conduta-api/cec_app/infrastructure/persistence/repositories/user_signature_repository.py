        ...
        with get_connection() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    ...
                    FROM comite_etica.user_signature_profiles
                    WHERE user_id = %s
                    """,
                    (_uuid(user_id),),
                )
    ...
    def upsert_profile(self, *, user_id: str, display_name: str) -> dict[str, Any]:
        with get_connection() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    """
                    INSERT INTO comite_etica.user_signature_profiles (user_id, display_name, updated_at)
                    VALUES (%s, %s, NOW())
        ...
        with get_connection() as conn:
            with conn.cursor() as cur:
                if display_name is not None:
                    cur.execute(
                        """
                        INSERT INTO comite_etica.user_signature_profiles (
                        ...
                        ON CONFLICT (user_id) DO UPDATE
                          SET signature_path = EXCLUDED.signature_path,
                              display_name = COALESCE(
                                ...
                                comite_etica.user_signature_profiles.display_name
                              ),
                              updated_at = NOW()
                        RETURNING user_id, display_name, signature_path, updated_at
                        """,
