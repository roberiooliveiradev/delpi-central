...
class MeetingMinuteRepository:
        ...
        *,
        ...
        if pending_for_user_id:
            clauses.append(
                """
                EXISTS (
                  SELECT 1 FROM comite_etica.meeting_minute_signers s
        ...
        with get_connection() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    f"SELECT COUNT(*) AS total FROM comite_etica.meeting_minutes m WHERE {where}",
                    params,
                )
                ...
                cur.execute(
                    ...
                    SELECT m.*,
                      (
                        SELECT COUNT(*) FROM comite_etica.meeting_minute_signers s
                        WHERE s.minute_id = m.id
                          AND s.version_id = m.current_version_id
                          AND s.status = 'signed'
                      ) AS signatures_done,
                    ...
                    FROM comite_etica.meeting_minutes m
                    WHERE {where}
    ...
    def get_minute(self, minute_id: str) -> dict[str, Any] | None:
        with get_connection() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    """
                    SELECT * FROM comite_etica.meeting_minutes
    ...
    def next_minute_number(self, conn, unit_code: str, year: int) -> str:
        ...
        with conn.cursor() as cur:
            cur.execute(
                """
                SELECT minute_number FROM comite_etica.meeting_minutes
        ...
        *,
        ...
        with get_connection() as conn:
            ...
            with conn.cursor() as cur:
                cur.execute(
                    """
                    INSERT INTO comite_etica.meeting_minutes (
                ...
                cur.execute(
                    """
                    INSERT INTO comite_etica.meeting_minute_versions (
                ...
                cur.execute(
                    """
                    UPDATE comite_etica.meeting_minutes
    ...
    def update_minute_draft(self, minute_id: str, fields: dict[str, Any], actor_user_id: str) -> dict[str, Any]:
        ...
        with get_connection() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    "SELECT * FROM comite_etica.meeting_minutes WHERE id = %s AND deleted_at IS NULL FOR UPDATE",
                    (_uuid(minute_id),),
                )
                before = cur.fetchone()
                if not before:
                    raise LookupError("Ata não encontrada.")
                cur.execute(
                    f"UPDATE comite_etica.meeting_minutes SET {', '.join(sets)} WHERE id = %s RETURNING *",
                    params,
                )
                after = cur.fetchone()
                self._audit(
        ...
        *,
        ...
        with get_connection() as conn:
            with conn.cursor() as cur:
                ...
                cur.execute(
                    """
                    UPDATE comite_etica.meeting_minute_versions
                    SET title = %s,
                        meeting_type = %s,
                        meeting_date = %s,
                        start_time = %s,
        ...
        with get_connection() as conn:
            with conn.cursor() as cur:
                ...
                cur.execute(
                    ...
                    FROM comite_etica.meeting_minute_versions WHERE minute_id = %s
                    """,
                    (minute["id"],),
                )
                ...
                cur.execute(
                    """
                    UPDATE comite_etica.meeting_minute_signers
        ...
        with get_connection() as conn:
            with conn.cursor() as cur:
                if version_id:
                    cur.execute(
                        """
                        SELECT * FROM comite_etica.meeting_minute_versions
                    ...
                    cur.execute(
                        """
                        SELECT v.* FROM comite_etica.meeting_minute_versions v
                        JOIN comite_etica.meeting_minutes m ON m.current_version_id = v.id
                        WHERE m.id = %s
                        """,
                        (_uuid(minute_id),),
                    )
            ...
            with conn.cursor() as cur:
                cur.execute(
                    ...
                    SELECT
                        ...
                        v.created_at,
                        v.title
                    FROM comite_etica.meeting_minute_versions v
                    LEFT JOIN comite_etica.user_signature_profiles usp
                        ON usp.user_id = v.created_by_user_id
                    LEFT JOIN LATERAL (
                        SELECT display_name
                        FROM comite_etica.meeting_minute_participants
                    ...
                    LEFT JOIN LATERAL (
                        SELECT display_name
                        FROM comite_etica.meeting_minute_signers
                        WHERE minute_id = v.minute_id
                          AND user_id = v.created_by_user_id
                        ORDER BY sign_order
                        LIMIT 1
    ...
    def replace_participants(
        ...
        with get_connection() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    "DELETE FROM comite_etica.meeting_minute_participants WHERE minute_id = %s",
                    (_uuid(minute_id),),
                )
                rows = []
                for index, item in enumerate(participants):
                    cur.execute(
                        """
                        INSERT INTO comite_etica.meeting_minute_participants (
    ...
    def list_participants(self, minute_id: str) -> list[dict[str, Any]]:
        with get_connection() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    """
                    SELECT * FROM comite_etica.meeting_minute_participants
                    WHERE minute_id = %s
        ...
        *,
        ...
        with get_connection() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    """
                    DELETE FROM comite_etica.meeting_minute_signers
                ...
                for index, item in enumerate(signers):
                    cur.execute(
                        """
                        INSERT INTO comite_etica.meeting_minute_signers (
        ...
        with get_connection() as conn:
            with conn.cursor() as cur:
                if version_id:
                    cur.execute(
                        """
                        SELECT * FROM comite_etica.meeting_minute_signers
                        WHERE minute_id = %s AND version_id = %s
                    ...
                    cur.execute(
                        """
                        SELECT s.* FROM comite_etica.meeting_minute_signers s
                        JOIN comite_etica.meeting_minutes m ON m.id = s.minute_id
        ...
        with get_connection() as conn:
            with conn.cursor() as cur:
                if idempotency_key:
                    cur.execute(
                        """
                        SELECT * FROM comite_etica.meeting_minute_signatures
                ...
                cur.execute(
                    """
                    INSERT INTO comite_etica.meeting_minute_signatures (
        ...
        with get_connection() as conn:
            with conn.cursor() as cur:
                if version_id:
                    ...
                    cur.execute(
                        """
                        SELECT sig.* FROM comite_etica.meeting_minute_signatures sig
                        JOIN comite_etica.meeting_minutes m ON m.id = sig.minute_id
    ...
    def replace_action_items(
        ...
        with get_connection() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    "DELETE FROM comite_etica.meeting_minute_action_items WHERE minute_id = %s",
                    (_uuid(minute_id),),
                )
                rows = []
                for index, item in enumerate(items):
                    cur.execute(
                        """
                        INSERT INTO comite_etica.meeting_minute_action_items (
    ...
    def list_action_items(self, minute_id: str) -> list[dict[str, Any]]:
        with get_connection() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    """
                    SELECT * FROM comite_etica.meeting_minute_action_items
                    WHERE minute_id = %s
            ...
            with conn.cursor() as cur:
                cur.execute(
                    ...
                    SELECT
                        ...
                        a.user_agent,
                        a.created_at
                    FROM comite_etica.meeting_minute_audit_logs a
                    LEFT JOIN comite_etica.user_signature_profiles usp
        ...
        *,
        ...
        with get_connection() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    """
                    INSERT INTO comite_etica.meeting_minute_attachments (
    ...
    def _audit(
        ...
        *,
        ...
        cur.execute(
            """
            INSERT INTO comite_etica.meeting_minute_audit_logs (
