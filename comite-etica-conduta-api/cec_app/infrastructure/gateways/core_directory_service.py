"""Gateway Core — resolve e-mails SMTP reais por user id (S2S)."""
...
logger = logging.getLogger("comite_etica.directory")


class CecCoreDirectoryService:
    def __init__(
        self,
        *,
    ...
    def lookup_emails_by_user_ids(self, user_ids: list[str]) -> dict[str, str]:
        """Retorna mapa user_id → e-mail (somente ativos com e-mail válido)."""
        ...
        try:
            ...
            logger.exception("cec_directory_lookup_failed")
            return {}

        if response.status_code >= 400:
            logger.warning(
                "cec_directory_lookup_rejected status=%s body=%s",
        ...
        try:
            ...
            logger.warning("cec_directory_lookup_invalid_json")
            return {}
