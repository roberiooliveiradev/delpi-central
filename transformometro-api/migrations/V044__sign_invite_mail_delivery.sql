-- Status de envio e entrega de e-mail por convite de assinatura (Transformômetro).

BEGIN;

ALTER TABLE transformometro.tm_meeting_minute_sign_invites
...
BEGIN
    IF NOT EXISTS (
        ...
        WHERE conname = 'ck_tm_sign_invites_mail_send_status'
          AND conrelid = 'transformometro.tm_meeting_minute_sign_invites'::regclass
    ) THEN
        ALTER TABLE transformometro.tm_meeting_minute_sign_invites
            ADD CONSTRAINT ck_tm_sign_invites_mail_send_status
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_constraint
        WHERE conname = 'ck_tm_sign_invites_mail_delivery_status'
          AND conrelid = 'transformometro.tm_meeting_minute_sign_invites'::regclass
    ) THEN
        ALTER TABLE transformometro.tm_meeting_minute_sign_invites
            ADD CONSTRAINT ck_tm_sign_invites_mail_delivery_status
...
CREATE INDEX IF NOT EXISTS idx_tm_sign_invites_mail_trace_pending
    ON transformometro.tm_meeting_minute_sign_invites (mail_sent_at DESC)
    WHERE mail_send_status = 'accepted'
      AND mail_delivery_status = 'trace_pending';

COMMENT ON COLUMN transformometro.tm_meeting_minute_sign_invites.mail_send_status IS
    'Resultado síncrono do sendMail Graph (accepted = HTTP 202).';
COMMENT ON COLUMN transformometro.tm_meeting_minute_sign_invites.mail_delivery_status IS
