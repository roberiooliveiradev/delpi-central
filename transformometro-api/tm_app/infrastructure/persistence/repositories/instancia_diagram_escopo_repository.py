...
class InstanciaDiagramEscopoRepository(PluginBaseRepository):
    ...
    def upsert(
        ...
        *,
        ...
        row = self.execute_returning_one(
            """
            INSERT INTO transformometro.instancia_diagrama_escopo (
                instancia_id, node_ids, inherit_all, include_boundary_edges, updated_at
            ) VALUES (%s::uuid, %s::jsonb, %s, %s, NOW())
            ON CONFLICT (instancia_id) DO UPDATE SET
                node_ids = EXCLUDED.node_ids,
                inherit_all = EXCLUDED.inherit_all,
                include_boundary_edges = EXCLUDED.include_boundary_edges,
                updated_at = NOW()
            RETURNING
                instancia_id, node_ids, inherit_all, include_boundary_edges,
                created_at, updated_at
            """,
            (instancia_id, json.dumps(node_ids), inherit_all, include_boundary_edges),
        )
        return row or {}
    ...
    def upsert_from_backup(self, row: dict[str, Any], *, auto_commit: bool = False) -> None:
        self.execute(
            ...
            (
                ...
                bool(row.get("include_boundary_edges", False)),
                row.get("created_at"),
                row.get("updated_at"),
            ),
