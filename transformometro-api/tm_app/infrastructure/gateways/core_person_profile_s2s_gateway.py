"""Gateway Core S2S — person-profile has_photo + photo bytes (never user JWT)."""
...
logger = logging.getLogger("transformometro.person_profile_s2s")

_LOOKUP_MAX_IDS = 50


class CorePersonProfileS2SGateway:
    def __init__(
        self,
        *,
    ...
    def _headers(self, *, accept: str = "application/json") -> dict[str, str]:
        ...
        headers = {
            "Accept": accept,
            "X-Delpi-Caller-App": "transformometro-api",
        }
        if token:
    ...
    def lookup_has_photo(self, user_ids: list[str]) -> dict[str, bool]:
        """Batch metadata: user_id → has_photo. Soft-fail empty when Core unavailable."""
        ...
        try:
            ...
            logger.exception("tm_person_profile_lookup_failed")
            return {}

        if response.status_code >= 400:
            logger.warning(
                "tm_person_profile_lookup_rejected status=%s body=%s",
        ...
        try:
            ...
            logger.warning("tm_person_profile_lookup_invalid_json")
            return {}
    ...
    def get_photo_bytes(self, user_id: str) -> tuple[bytes, str, str] | None:
        ...
        try:
            ...
            logger.exception("tm_person_profile_photo_failed user_id=%s", uid)
            return None
        ...
        if response.status_code >= 400:
            logger.warning(
                "tm_person_profile_photo_rejected status=%s user_id=%s",
                response.status_code,
                uid,
            )
            return None
