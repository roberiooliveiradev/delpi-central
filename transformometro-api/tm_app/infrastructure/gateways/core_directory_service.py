...
logger = logging.getLogger("transformometro.directory")


class TmCoreDirectoryService:
    def __init__(
        self,
        *,
    ...
    def lookup_emails_by_user_ids(self, user_ids: list[str]) -> dict[str, str]:
        ...
        try:
            ...
            logger.exception("tm_directory_lookup_failed")
            return {}

        if response.status_code >= 400:
            logger.warning(
                "tm_directory_lookup_rejected status=%s body=%s",
        ...
        try:
            ...
            logger.warning("tm_directory_lookup_invalid_json")
            return {}
