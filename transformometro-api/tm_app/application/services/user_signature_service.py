...
class UserSignatureService:
    ...
    @staticmethod
    def _has_sign_permission(user: Any) -> bool:
        permissions = set(getattr(user, "permissions", None) or getattr(user, "roles", None) or [])
        return not permissions or "transformometro.atas.sign" in permissions or "transformometro.atas.manage" in permissions

    def _assert_sign(self, user: Any) -> None:
        if not self._has_sign_permission(user):
            raise PermissionError("Sem permissão para assinar atas.")
    ...
    def _payload(self, row: dict[str, Any] | None, user_id: str) -> dict[str, Any]:
        ...
        return {"user_id": str(row.get("user_id") or user_id), "display_name": str(row.get("display_name") or ""),
    ...
    def get_me(self, user: Any) -> dict[str, Any]:
        self._assert_sign(user); user_id = self._user_id(user)
    ...
    def update_display_name(self, user: Any, display_name: str) -> dict[str, Any]:
        self._assert_sign(user); user_id = self._user_id(user); name = (display_name or "").strip()
        if not name: raise ValueError("Informe o nome para assinatura.")
        if len(name) > 200: raise ValueError("Nome para assinatura excede 200 caracteres.")
    ...
    def save_image(self, user: Any, raw: bytes) -> dict[str, Any]:
        self._assert_sign(user); user_id = self._user_id(user)
        try: path = self.storage.save_png(user_id=user_id, raw=raw)
        except TmAtaStorageError as exc: raise ValueError(str(exc)) from exc
        existing = self.repo.get_by_user(user_id) or {}
        name = str(existing.get("display_name") or getattr(user, "name", None) or getattr(user, "preferred_username", None) or "Signatário").strip()
        return self._payload(self.repo.update_signature_path(user_id=user_id, signature_path=path, display_name=name), user_id)

    def read_image(self, user: Any) -> bytes:
        self._assert_sign(user)
        try: return self.storage.read(self._user_id(user))
        except TmAtaStorageError as exc: raise LookupError(str(exc)) from exc
