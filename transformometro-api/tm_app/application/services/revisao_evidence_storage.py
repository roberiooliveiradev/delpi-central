...
class RevisaoEvidenceStorageError(ValueError):
    pass


class RevisaoEvidenceStorage:
    """Storage persistente de evidências de revisão (metadado no Postgres + binário no volume)."""

    def __init__(self, base_dir: str | None = None) -> None:
        self.base_dir = Path(base_dir or settings.TM_REVISION_EVIDENCE_UPLOAD_DIR)
        self.base_dir.mkdir(parents=True, exist_ok=True)

    def validate_upload(self, *, mime_type: str | None, size_bytes: int) -> None:
        if size_bytes <= 0:
            raise RevisaoEvidenceStorageError("Arquivo vazio.")
        if size_bytes > MAX_EVIDENCE_BYTES:
            raise RevisaoEvidenceStorageError("Arquivo excede o limite de 25 MB.")
        normalized = (mime_type or "").lower()
        if normalized not in ALLOWED_MIME_TYPES:
            raise RevisaoEvidenceStorageError(
    ...
    def save(
        ...
        *,
        ...
        target_dir = self.base_dir / revisao_id
        target_dir.mkdir(parents=True, exist_ok=True)
        (target_dir / stored_name).write_bytes(content)
        return stored_name

    def resolve_file(self, *, revisao_id: str, stored_name: str) -> Path:
        normalized_name = (stored_name or "").strip()
        if not normalized_name:
            raise RevisaoEvidenceStorageError("Arquivo não encontrado.")

        base = self.base_dir.resolve()
        path = (self.base_dir / (revisao_id or "").strip() / normalized_name).resolve()
        if not str(path).startswith(str(base)):
            raise RevisaoEvidenceStorageError("Caminho de arquivo inválido.")
        if path.is_file():
            return path
        raise RevisaoEvidenceStorageError("Arquivo não encontrado.")
