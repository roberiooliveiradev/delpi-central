...
TEO_MCP_INSTRUCTIONS = (
    ...
    "Use MCP tools for READ, PREPARE, and COMMIT over Transformômetro capabilities. "
    "Material writes require PREPARE → opaque proposal_handle → commit_proposal. "
    "Never invent or alter the prepared change on commit; only pass proposal_handle. "
    "Conventional ENTITY CRUD: prepare_record_change then commit_proposal "
    "(process_document and other catalog entities). "
    "Business WORKFLOWS keep specialized prepare_* tools, then the same commit_proposal. "
    ...
    "Before writes: get_catalog → registration_guide / capability_surface / entity_schemas. "
    ...
    "commit_proposal(proposal_handle). Incomplete packages have "
    ...
    "Tool success (isError=false) on commit means verified authoritative read-back. "
    ...
    "Never use execute_capability / invoke_tool / generic SQL or HTTP. "
