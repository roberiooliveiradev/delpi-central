...
SCHEMA_NAME = "supplies"
MIGRATIONS_DIR = Path(__file__).resolve().parents[3] / "migrations"


class MigrationError(RuntimeError):
    """Erro de execução de migrations da Supplies."""
...
def run_migrations() -> None:
    ...
    with get_connection() as conn:
        ...
        if not pending:
            print("[supplies] Nenhuma migration pendente.")
            return

        print("[supplies] Executando migrations...")
        for path in pending:
            apply_migration(conn, path)

        print("[supplies] Migrations aplicadas com sucesso.")
...
def show_status() -> None:
    ...
    with get_connection() as conn:
        ...
        print("Status das migrations da Supplies:")
        for path in files:
            version, name = parse_version_and_name(path)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Runner de migrations Postgres da Supplies API.",
    )
    parser.add_argument(
        "command",
        choices=["up", "status"],
