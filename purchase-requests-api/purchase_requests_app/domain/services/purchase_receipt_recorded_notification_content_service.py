"""Declarative texts for purchase-receipt-recorded portal notifications."""
...
_CONTENT_PATH = (
    ...
    / "content"
    / "pt-BR"
    / "purchase_receipt_recorded_notification.json"
)
...
class PurchaseReceiptRecordedNotificationContentService:
    @classmethod
    ...
    def event_type(cls) -> str:
        return str(_load().get("eventType") or "purchase_receipt_recorded").strip()

    @classmethod
    def notification_type(cls) -> str:
        return str(_load().get("type") or "success").strip() or "success"
    ...
    @classmethod
    def format_entry_date(cls, raw: str | None) -> str:
        value = (raw or "").strip()
        if not value:
        ...
        for fmt in ("%Y-%m-%d", "%Y%m%d"):
            try:
                parsed = datetime.strptime(
                    value[:10] if fmt == "%Y-%m-%d" else value[:8],
                    fmt,
                )
    ...
    @classmethod
    def format_quantity(cls, raw: float | int | str | None) -> str:
    ...
    @classmethod
    def format_title(cls, *, request_number: str, invoice_number: str) -> str:
        template = str(
            _load().get("titleTemplate") or "Recebimento da SC {request} — NF {invoice}"
        )
        return template.format(
            request=(request_number or "").strip() or cls.empty_value(),
            invoice=(invoice_number or "").strip() or cls.empty_value(),
        )
    ...
    def format_message(
        ...
        *,
        ...
        template = str(
            ...
            "Fornecedor {supplier} · {quantity} un. em {entry_date}"
        )
        ...
        return template.format(
            ...
            quantity=cls.format_quantity(quantity),
            entry_date=cls.format_entry_date(entry_date),
        )
