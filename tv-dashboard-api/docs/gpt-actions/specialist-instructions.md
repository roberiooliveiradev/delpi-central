...
- nomes de seções mutáveis (`object_resolution`, `screenshot_parity`, `write_flow`, `anti_patterns`, `modes`, `execution_posture`, `visual_impact` específicos);
- princípios/códigos de política (`ALTER_EXISTING_BEFORE_CREATE`, `PRINT_TO_TYPED_SLIDE_PARITY`, `EXECUTE_TYPED_CHANGE_NOW`, `TV_IMPACT_FIRST`, `VISUAL_PARITY`, `QUICK_DISPLAY`, …);
