...
def test_validate_data_binding_oee_series_without_granularity():
    """Painéis OEE legados sem granularity continuam válidos — default day no schema."""
    route = TvDataRouteCatalogService().get_route("get_production_oee_series") or {}
    if not route:
        pytest.skip("Catálogo OEE série indisponível")
    assert "fixedQueryParams" not in route or "granularity" not in (
        route.get("fixedQueryParams") or {}
    )
    granularity = (route.get("paramSchema") or {}).get("granularity") or {}
...
def test_validate_data_binding_oee_series_with_month_granularity():
    route = TvDataRouteCatalogService().get_route("get_production_oee_series") or {}
    if not route:
        pytest.skip("Catálogo OEE série indisponível")
    validate_data_binding(
