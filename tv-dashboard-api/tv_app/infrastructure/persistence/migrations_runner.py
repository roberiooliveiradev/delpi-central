...
def ensure_migrations_table(conn: Any) -> None:
    with conn.cursor() as cur:
        cur.execute(f'CREATE SCHEMA IF NOT EXISTS "{TV_SCHEMA_NAME}";')
        cur.execute(
            f"""
            CREATE TABLE IF NOT EXISTS "{TV_SCHEMA_NAME}".schema_migrations (
...
def get_applied_migrations(conn: Any) -> dict[str, dict[str, Any]]:
    with conn.cursor() as cur:
        cur.execute(
            ...
            FROM "{TV_SCHEMA_NAME}".schema_migrations
...
def apply_migration(conn: Any, path: Path) -> None:
    ...
    try:
        with conn.cursor() as cur:
            ...
            cur.execute(
                f"""
                INSERT INTO "{TV_SCHEMA_NAME}".schema_migrations (version, name, checksum)
                VALUES (%s, %s, %s)
                """,
...
def validate_migration_history(conn: Any, files: list[Path]) -> None:
    ...
    for path in files:
        ...
        if version in applied and checksum != applied[version]["checksum"]:
            raise MigrationError(
                f"Checksum divergente para {path.name}. A migration já aplicada foi alterada."
            )
...
def run_migrations() -> None:
    ...
    with get_connection() as conn:
        ...
        applied = get_applied_migrations(conn)
        pending = [path for path in files if parse_version_and_name(path)[0] not in applied]
        if not pending:
            print("[tv-dashboard] Nenhuma migration pendente.")
            return
        print("[tv-dashboard] Executando migrations...")
        for path in pending:
            apply_migration(conn, path)
        print("[tv-dashboard] Migrations aplicadas com sucesso.")


def main() -> None:
    parser = argparse.ArgumentParser(description="Runner de migrations TV Dashboard API.")
