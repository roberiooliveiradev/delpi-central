...
from cipa_app.application.services.token_service import generate_public_token, hash_token
from cipa_app.config import settings

logger = logging.getLogger("cipa.sign_invites")
...
class CipaMeetingMinuteSignInviteService:
    def __init__(
        self,
        ...
        *,
        ...
        public_sign_path: str | None = None,
    ) -> None:
        self.repo = repo
        self.ttl_days = int(
            ttl_days
            if ttl_days is not None
            else settings.CIPA_ATA_SIGN_INVITE_TTL_DAYS
        )
        ...
        self.public_sign_path = (
            public_sign_path
            if public_sign_path is not None
            else settings.CIPA_PUBLIC_SIGN_PATH
        ).rstrip("/")

    def build_public_sign_url(self, raw_token: str) -> str:
        path = f"{self.public_sign_path}/{raw_token}"
    ...
    def _try_remap_stale_signer(
        ...
        *,
        ...
        logger.info(
            "cipa_sign_invite_rebound invite_id=%s from_signer=%s to_signer=%s minute_id=%s",
            invite.get("id"),
            signer.get("id"),
            match.get("id"),
            minute.get("id"),
