...
from cipa_app.application.services.sign_pending_mail_service import build_graph_mail_client
...
from cipa_app.infrastructure.providers.microsoft_graph.microsoft_graph_message_trace_client import (
    MicrosoftGraphMessageTraceClient,
)

logger = logging.getLogger("cipa.mail.trace")
...
class SignInviteMailTraceReconciliationService:
        ...
        *,
        ...
        if trace_client is not None:
            ...
            self.trace_client = MicrosoftGraphMessageTraceClient(
                ...
                timeout_seconds=float(settings.CIPA_GRAPH_HTTP_TIMEOUT_SECONDS or "30"),
            )
        self.enabled = (
            settings.CIPA_SIGN_INVITE_MAIL_TRACE_ENABLED
            if enabled is None
            else enabled
        )
        self.batch_limit = (
            int(settings.CIPA_SIGN_INVITE_MAIL_TRACE_BATCH_LIMIT or "50")
    ...
    def execute(self) -> dict[str, Any]:
        ...
        try:
            ...
            logger.warning("cipa_mail_trace_skipped_graph_not_configured")
    ...
    def reconcile_invite(self, invite: dict[str, Any]) -> bool:
        ...
        try:
            ...
            logger.warning(
                "cipa_mail_trace_query_failed invite=%s error=%s",
                invite_id,
                str(exc)[:200],
            )
            return False
        ...
        if trace_key:
            try:
                ...
                logger.warning(
                    "cipa_mail_trace_details_failed invite=%s trace=%s error=%s",
                    invite_id,
                    trace_key,
                    str(exc)[:200],
                )
