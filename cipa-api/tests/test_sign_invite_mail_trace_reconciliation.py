...
def test_reconcile_invite_marks_unknown_when_trace_window_expired():
    repo = MagicMock()
    trace_client = MagicMock()
    ...
    assert updated is True
    trace_client.list_message_traces.assert_not_called()
    assert (
        repo.update_invite_mail_delivery_result.call_args.kwargs["mail_delivery_status"]
        == MAIL_DELIVERY_UNKNOWN
    )
