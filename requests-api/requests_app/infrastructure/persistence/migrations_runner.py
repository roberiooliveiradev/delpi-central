...
SCHEMA_NAME = "my_requests"
MIGRATIONS_DIR = Path(__file__).resolve().parents[3] / "migrations"


class MigrationError(RuntimeError):
    """Erro de execução de migrations da Requests API."""
...
def run_migrations() -> None:
    ...
    with get_connection() as conn:
        ...
        if not pending:
            print("[my-requests] Nenhuma migration pendente.")
            return

        print("[my-requests] Executando migrations...")
        for path in pending:
            apply_migration(conn, path)

        print("[my-requests] Migrations aplicadas com sucesso.")
...
def show_status() -> None:
    ...
    with get_connection() as conn:
        ...
        print("Status das migrations da Requests API:")
        for path in files:
            version, name = parse_version_and_name(path)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Runner de migrations Postgres da Requests API.",
    )
    parser.add_argument(
        "command",
        choices=["up", "status"],
