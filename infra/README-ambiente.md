...
Uploads de evidências do plugin **quality-action-plans** e anexos da **auditoria-5s** (foto por critério na avaliação + evidências NC antes/depois) ficam no filesystem do container `api-delpi`, paths padrão `/app/data/pac-evidences`, `/app/data/audit-5s-responses` e `/app/data/audit-5s-nc`.
...
| `CORE_USER_AVATAR_UPLOAD_DIR` | `/app/data/core-user-avatars` | `${DELPI_DATA_HOST_DIR}/core-user-avatars` |
...
Definir em `infra/.env`. Compose (dev e prod) repassa `KIMI_*` a `transformometro-api` **e** a `minha-delpi-ai-api`. Detalhes: [kimi.md](../transformometro-api/docs/meeting-minutes/kimi.md) · [tutorial LLM externo](../minha-delpi-ai-api/docs/operations/tutorial-conectar-llm-externo.md).
...
Após o upload, a API aplica **faststart** (MP4) e gera **poster JPEG** via `ffmpeg` na imagem Docker (`tv-dashboard-api/Dockerfile`). Assets legados: `POST /playlists/{id}/media/{assetId}/optimize`.
Em produção atrás do **Cloudflare** (teto ~100 MB/request), uploads maiores usam sessão em partes (`maxEdgeChunkBytes`, default 90 MB).
...
JWKS em host network: **`PRODUCTION_PULSE_KEYCLOAK_JWKS_URL=http://127.0.0.1/auth/realms/delpi/protocol/openid-connect/certs`** (default no `docker-compose.dev.yml`). O alias Docker `keycloak` **não** resolve com `network_mode: host` — se a API usar `KEYCLOAK_JWKS_URL=http://keycloak:8080/...`, login autenticado falha (timeout/401) ao buscar certs.
