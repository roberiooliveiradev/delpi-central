...
SCHEMA_NAME = "travel_expenses"
MIGRATIONS_DIR = Path(__file__).resolve().parents[3] / "migrations"


class MigrationError(RuntimeError):
    """Travel expenses migration error."""
...
def run_migrations() -> None:
    ...
    with get_connection() as conn:
        ...
        if not pending:
            print("[travel-expenses] Nenhuma migration pendente.")
            return
        print("[travel-expenses] Executando migrations...")
        for path in pending:
            apply_migration(conn, path)
        print("[travel-expenses] Migrations aplicadas com sucesso.")
...
def show_status() -> None:
    ...
    with get_connection() as conn:
        ...
        validate_migration_history(conn, files)
        applied = get_applied_migrations(conn)
        print("Status das migrations de Despesas de Viagem:")
        for path in files:
            version, name = parse_version_and_name(path)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Runner de migrations Postgres da travel-expenses-api.",
    )
