...
def create_totvs_connection() -> pyodbc.Connection:
    return pyodbc.connect(build_totvs_connection_string())
...
class TotvsConnectionPool:
    ...
    def acquire(self, *, timeout_seconds: float = 60.0) -> pyodbc.Connection:
        ...
        while True:
            try:
                ...
                logger.debug("totvs_pool reuse wait_ms=%.0f", (time.perf_counter() - started) * 1000)
                return connection
            except queue.Empty:
                pass
    ...
    def release(self, connection: pyodbc.Connection | None, *, discard: bool = False) -> None:
...
def get_totvs_connection_pool() -> TotvsConnectionPool | None:
    if not settings.TOTVS_POOL_ENABLED:
        return None
    ...
    with _pool_lock:
        if _pool is None:
            _pool = TotvsConnectionPool(max_size=settings.TOTVS_POOL_MAX_SIZE)
            logger.info(
                "totvs_pool enabled max_size=%d",
                settings.TOTVS_POOL_MAX_SIZE,
            )
        return _pool
