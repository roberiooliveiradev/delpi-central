...
def _is_dashboard_goals_integration_path(path: str) -> bool:
    normalized = normalize_path(path)
    return "integrations/dashboard-goals" in normalized


async def jwt_middleware(request: Request, call_next):
    if _is_dashboard_goals_integration_path(request.url.path) and request_has_valid_internal_service_token(
        request
    ):
