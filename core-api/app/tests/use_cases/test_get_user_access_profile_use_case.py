...
def test_execute_merges_direct_and_group_roles_with_group_id_in_source():
    user_id = uuid4()
    role_id = uuid4()
    group_id = uuid4()
    ...
    result = GetUserAccessProfileUseCase(uow).execute(user_id, is_superadmin=False)
    ...
    group_source = next(
        source for source in result["roles"][0]["sources"] if source["type"] == "group"
    )
    assert group_source["groupName"] == "Equipe Chat"
    assert group_source["groupId"] == str(group_id)
