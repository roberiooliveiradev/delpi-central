...
class CoreUserAvatarStorageError(ValueError):
    pass
...
class CoreUserAvatarStorage:
    """Binários de foto de usuário — volume CORE_USER_AVATAR_UPLOAD_DIR."""

    def __init__(self, base_dir: str | None = None) -> None:
        self.base_dir = Path(base_dir or Config.CORE_USER_AVATAR_UPLOAD_DIR)
    ...
    def validate_upload(self, *, mime_type: str | None, size_bytes: int) -> None:
        if size_bytes <= 0:
            raise CoreUserAvatarStorageError("Arquivo vazio.")
        if size_bytes > MAX_USER_PHOTO_BYTES:
            raise CoreUserAvatarStorageError("Arquivo excede o limite de 2 MB.")
        normalized = (mime_type or "").lower()
        if normalized not in ALLOWED_USER_PHOTO_MIME_TYPES:
            raise CoreUserAvatarStorageError(
    ...
    def resolve_path(self, storage_key: str) -> Path:
        ...
        if not key or ".." in key.split("/"):
            raise CoreUserAvatarStorageError("Chave de armazenamento inválida.")
    ...
    def delete(self, storage_key: str | None) -> None:
        ...
        except CoreUserAvatarStorageError:
            return
