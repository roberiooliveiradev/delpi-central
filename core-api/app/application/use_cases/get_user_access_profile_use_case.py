...
class GetUserAccessProfileUseCase:
    """Perfil de acesso de um usuário: papéis, permissões por papel e apps liberados."""
        ...
        for group_id in self.uow.user_groups.list_group_ids(user_id):
            ...
            for role_id in self.uow.group_roles.list_role_ids(group_id):
                ...
                self._merge_role(
                    roles_by_id,
                    role_id=role_id,
                    source={"type": "group", "groupName": group.name, "groupId": str(group.id)},
                )
