# app/infrastructure/persistence/plugins/repositories/internal_nc/postgres_internal_nc_details_repository.py
...
class PostgresInternalNcDetailsRepository(
    PluginBaseRepository,
    InternalNcDetailsRepositoryPort,
):
    def get_full_details(self, nonconformity_id: str) -> dict | None:
        ...
        attachments = self.fetch_all(
            ...
            SELECT
                ...
                uploaded_by_user_id,
                uploaded_at
            FROM quality.internal_nc_attachments
            WHERE nonconformity_id = %s
               OR action_id IN (
                    SELECT id
                    FROM quality.internal_nc_actions
        ...
        audit_events = self.fetch_all(
            ...
            SELECT
                ...
                payload_json,
                created_at
            FROM quality.internal_nc_audit_events
            WHERE nonconformity_id = %s
            ORDER BY created_at ASC
            """,
            (nonconformity_id,),
