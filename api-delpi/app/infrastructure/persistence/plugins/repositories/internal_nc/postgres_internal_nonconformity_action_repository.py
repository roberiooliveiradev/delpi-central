# app/infrastructure/persistence/plugins/repositories/internal_nc/postgres_internal_nonconformity_action_repository.py
...
class PostgresInternalNonconformityActionRepository(
    PluginBaseRepository,
    InternalNonconformityActionRepositoryPort,
):
    def create(
        ...
        row = self.execute_returning_one(
            """
            INSERT INTO quality.internal_nc_actions (
                id,
                nonconformity_id,
                root_cause_id,
                action_type,
    ...
    def get_by_id(
        ...
        row = self.fetch_one(
            ...
            SELECT
                ...
                created_at,
                updated_at
            FROM quality.internal_nc_actions
            WHERE id = %s
            """,
            (action_id,),
        )
    ...
    def update(
        ...
        row = self.execute_returning_one(
            """
            UPDATE quality.internal_nc_actions
               SET root_cause_id = %s,
                   action_type = %s,
                   title = %s,
                   description = %s,
    ...
    def _to_entity(self, row: dict[str, Any]) -> InternalNonconformityAction:
        return InternalNonconformityAction(
            id=str(row["id"]),
