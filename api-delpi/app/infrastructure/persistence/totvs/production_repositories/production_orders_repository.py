...
class ProductionOrdersRepository(
    ...
    def fetch_operation_materials(
        ...
        *,
        ...
        sql = operation_materials_sql(order_count=1, filter_operation=True)
    ...
    def fetch_operation_materials_batch(
        ...
        *,
        ...
        """Empenhos SD4 de várias OPs de uma vez, agregados por OP, operação e componente.

        Mesma definição de empenho da consulta unitária; quem chama filtra os
        pares OP+operação que interessam.
        """
        orders = tuple(production_orders or ())
        if not orders:
            return []

        sql = operation_materials_sql(order_count=len(orders), filter_operation=False)
        params = (*orders, branch)
        with self as repo:
            return repo.execute_query(sql, params)
