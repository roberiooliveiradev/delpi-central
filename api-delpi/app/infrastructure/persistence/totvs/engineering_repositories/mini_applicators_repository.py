...
class MiniApplicatorsRepository(BaseRepository, MiniApplicatorsRepositoryPort):
        ...
        return f"""
            EXISTS (
                ...
                WHERE G.D_E_L_E_T_ = ''
                  ...
                  AND (
                        RTRIM(LTRIM(G.G1_FIM)) = ''
                        OR RTRIM(LTRIM(G.G1_FIM)) > CONVERT(CHAR(8), GETDATE(), 112)
                      )
    ...
    def list_pecas_reposicao(
        ...
        where_clauses = [
            "SB1.D_E_L_E_T_ = ''",
            peca_reposicao_scope_sql(alias="SB1"),
        ]
        params: list = []

        if request.codigo:
            where_clauses.append(peca_codigo_filter_sql(alias="SB1"))
            params.append(codigo_prefix_pattern(request.codigo))

        if request.descricao:
            append_descricao_terms(
                column_sql="RTRIM(LTRIM(SB1.B1_DESC))",
                descricao=request.descricao,
                where_clauses=where_clauses,
                params=params,
            )
        ...
        sort_columns = {
            "codigo": "RTRIM(LTRIM(SB1.B1_COD))",
            "descricao": "RTRIM(LTRIM(SB1.B1_DESC))",
        }
        sort_key = (request.sort_by or "codigo").strip().lower()
        sort_column = sort_columns.get(sort_key, sort_columns["codigo"])
        sort_direction = "DESC" if str(request.sort_dir or "asc").lower() == "desc" else "ASC"
        ...
        list_query = f"""
            SELECT
                SB1.R_E_C_N_O_ AS id,
                RTRIM(LTRIM(SB1.B1_COD)) AS codigo,
                RTRIM(LTRIM(SB1.B1_DESC)) AS descricao
            FROM SB1010 SB1 WITH (NOLOCK)
            WHERE {where_sql}
            ORDER BY {sort_column} {sort_direction}
            OFFSET ? ROWS FETCH NEXT ? ROWS ONLY
