...
# Tempo previsto estável: setup + HY_TEMPAD × qtd (não HY_TEMPOM parcial da view).
# Ver production_tempo_previsto.py
_SETUP_HORAS_SQL = """
...
_TEMPO_PREVISTO_SQL = f"""
CASE
    WHEN ({_UNIT_HOURS_SQL}) IS NOT NULL
     AND TRY_CAST(EF.QTD_APONTADA AS FLOAT) IS NOT NULL
     AND TRY_CAST(EF.QTD_APONTADA AS FLOAT) > 0
    THEN ROUND(
        ({_SETUP_HORAS_SQL})
        + ({_UNIT_HOURS_SQL}) * TRY_CAST(EF.QTD_APONTADA AS FLOAT),
        6
    )
...
_EFICIENCIA_PERCENTUAL_SQL = f"""
CASE
    WHEN TRY_CAST(EF.TEMPO_REAL_HORAS AS FLOAT) > 0
     AND ({_TEMPO_PREVISTO_SQL}) IS NOT NULL
    THEN ROUND(
        ({_TEMPO_PREVISTO_SQL}) * 100.0 / TRY_CAST(EF.TEMPO_REAL_HORAS AS FLOAT),
        2
    )
    ELSE NULL
END
...
_TEMPO_GANHO_PERDIDO_SQL = f"""
CASE
    WHEN ({_TEMPO_PREVISTO_SQL}) IS NOT NULL
     AND TRY_CAST(EF.TEMPO_REAL_HORAS AS FLOAT) IS NOT NULL
    THEN ROUND(
        ({_TEMPO_PREVISTO_SQL}) - TRY_CAST(EF.TEMPO_REAL_HORAS AS FLOAT),
        6
    )
...
EF_FABRIL_ITEMS_SELECT = f"""
    ...
    {_TEMPO_PREVISTO_SQL} AS TEMPO_PREVISTO_HORAS,
    {_EFICIENCIA_PERCENTUAL_SQL} AS EFICIENCIA_PERCENTUAL,
    TRY_CAST(EF.VALOR_MOD_HORA AS FLOAT) AS VALOR_MOD_HORA,
    {_TEMPO_GANHO_PERDIDO_SQL} AS TEMPO_GANHO_PERDIDO_HORAS,
