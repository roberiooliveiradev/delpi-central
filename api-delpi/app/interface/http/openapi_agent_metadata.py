...
LMP_NONCONFORMITY_UPDATE = agent_route(
    ...
    description="Full update of an LMP nonconformity and its product lines.",
...
PRODUCTION_OTD = agent_route(
    ...
    description=(
        ...
        "de ordens (SC2010: C2_DATPRF × C2_DATRF). "
        "Universo: OP mãe finalizada ou em andamento já atrasada "
        "(C2_DATRF vazio e C2_DATPRF anterior a hoje). Período filtra pela data prevista. "
...
COMMERCIAL_PROPOSAL_COLLABORATOR_SUMMARY = agent_route(
    ...
    description=(
        "SQL aggregation of commercial proposals (OV) by seller for the period filters. "
        "Open/lost use opening date (AD1_DATA); won uses acceptance date (AD1_DTASSI / AD1_DTFIM) — "
        "same semantics as the proposals list. No pagination; independent from the list status filter."
    ),
