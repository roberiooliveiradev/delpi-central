...
from app.composition.commercial_composer import (
    ...
    build_get_sales_order_otd_series_by_customer_use_case,
...
def get_weg_rol_target_pct(
    ...
    try:
        ...
        use_case = build_get_weg_rol_target_use_case()
        ...
        return api_delpi_success(
            result,
            operation_id="get_weg_rol_target_pct",
            message="WEG ROL target percentage fetched successfully.",
            fields=kpi_fields(COMMERCIAL_ROL_FIELD_LABELS),
        )

    except ValueError as exc:
        log_error(f"Validation error while fetching WEG ROL target: {exc}")
        return error_response(str(exc), status_code=400)

    except Exception as exc:
        log_error(f"Error while fetching WEG ROL target: {exc}")
        return error_response(
            "Internal error while fetching WEG ROL target.",
            status_code=500,
        )
...
@require_any_permission(KPI_COMMERCIAL_ACCESS)
def get_new_business_rol_target_pct(
    branch: Optional[str] = BRANCH_QUERY_OPTIONAL(),
    start_date: Optional[str] = Query(None),
    end_date: Optional[str] = Query(None),
):
    try:
        concrete_branch = optional_concrete_branch(branch)
        use_case = build_get_new_business_rol_target_use_case()
        ...
        return api_delpi_success(
            result,
            operation_id="get_new_business_rol_target_pct",
            message="New business ROL target percentage fetched successfully.",
            fields=kpi_fields(COMMERCIAL_ROL_FIELD_LABELS),
        )

    except ValueError as exc:
        log_error(f"Validation error while fetching new business ROL target: {exc}")
        return error_response(str(exc), status_code=400)

    except Exception as exc:
        log_error(f"Error while fetching new business ROL target: {exc}")
        return error_response(
            "Internal error while fetching new business ROL target.",
            status_code=500,
        )
...
def get_sales_order_otd_by_customer(
    ...
    customer_code_stores: Optional[str] = Query(
        None,
...
@router.get(
    "/sales-order-otd/series-by-customer",
    **OpenApiAgentMetadataBuilder.from_contract(
        "get_sales_order_otd_series_by_customer",
        path="/commercial/sales-order-otd/series-by-customer",
    ),
)
@require_any_permission(KPI_COMMERCIAL_ACCESS)
def get_sales_order_otd_series_by_customer(
    ...
    try:
        ...
        result = build_get_sales_order_otd_series_by_customer_use_case().execute(request)
        return api_delpi_success(
            result,
            operation_id="get_sales_order_otd_series_by_customer",
            message="Sales order OTD series by customer fetched successfully.",
            fields=kpi_fields(
                COMMERCIAL_SALES_ORDER_OTD_FIELD_LABELS,
                COMMERCIAL_SALES_ORDER_OTD_ANALYSIS_FIELD_LABELS,
            ),
        ...
        log_error(
            f"Validation error while fetching sales order OTD series by customer: {exc}"
        )
        return error_response(str(exc), status_code=400)
    except Exception as exc:
        log_error(f"Error while fetching sales order OTD series by customer: {exc}")
        return error_response(
            "Internal error while fetching sales order OTD series by customer.",
            status_code=500,
        )
