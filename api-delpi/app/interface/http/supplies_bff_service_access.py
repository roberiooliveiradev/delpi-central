...
PURCHASE_REQUESTS_API_CALLER = "purchase-requests-api"


def is_trusted_internal_caller(expected_caller: str) -> bool:
    request = get_request()
    if request is None:
        return False
    if (get_caller_app() or "").strip() != expected_caller.strip():
        return False
...
def is_trusted_supplies_bff_call(*, supplies_department_only: bool = False) -> bool:
    if not is_trusted_internal_caller(SUPPLIES_BFF_CALLER):
        return False
...
def require_any_permission_or_trusted_caller(permission_codes, *, caller: str):
    """User permission, or one named internal caller with a valid service token."""
    ...
    def decorator(fn):
        ...
        if inspect.iscoroutinefunction(fn):
            ...
            async def async_wrapper(*args, **kwargs):
                if is_trusted_internal_caller(caller):
