"""Testes de autorização por filial — Emissão de Notas Fiscais."""
...
_MOD = (
    "app.interface.http.routes.invoice_issuance.invoice_issuance_branch_access"
)
...
def test_global_view_allows_both_branches() -> None:
    user = SimpleNamespace(is_superadmin=False, permissions=[INVOICE_ISSUANCE_VIEW])
...
def test_process_allows_both_branches() -> None:
    user = SimpleNamespace(is_superadmin=False, permissions=[INVOICE_ISSUANCE_PROCESS])
    get_user, has_perm = _patch_user(user)
    with get_user, has_perm:
        assert branch_view_allowed("02") is True
...
def test_filial_01_only() -> None:
    user = SimpleNamespace(
        is_superadmin=False, permissions=[INVOICE_ISSUANCE_VIEW_FILIAL_01]
    )
    get_user, has_perm = _patch_user(user)
    with get_user, has_perm:
