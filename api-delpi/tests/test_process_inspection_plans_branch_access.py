"""Testes de autorização por filial — process inspection plans."""
...
def test_branch_view_allowed_with_global_view_permission() -> None:
    user = SimpleNamespace(is_superadmin=False, permissions=[INSPECOES_PROCESSO_VIEW])
...
def test_branch_view_allowed_with_filial_01_only() -> None:
    user = SimpleNamespace(
        is_superadmin=False, permissions=[INSPECOES_PROCESSO_VIEW_FILIAL_01]
    )
...
def test_branch_access_error_all_requires_consolidated() -> None:
    with patch(
...
def test_branch_access_error_all_allowed() -> None:
    with patch(
        "app.interface.http.branch_access_gate.BranchAccessGate.consolidated_view_allowed",
        return_value=True,
    ):
