...
| GET | `/production/on_time_delivery_pct` | OTD produção (%) — OPs de PA (`SB1010.B1_TIPO = 'PA'`) com `C2_PRODUTO` prefixo 9026, 8000 ou 8001 e sequência mãe `001`. |
| GET | `/production/otd` | OTD produção — resumo, listagem paginada de OPs de PA (mesmos prefixos e sequência `001`) e filtro `status` (`on_time` / `late`). |
