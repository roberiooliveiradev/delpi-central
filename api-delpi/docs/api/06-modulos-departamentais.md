...
Fila de solicitações de emissão de NF de saída (Postgres plugins). Lookups TOTVS SA1/SA2/SB1/SA4 via **`/request-lookups`** (E18).  
Plugin legado: `invoice-issuance` (MFE fora do Compose). Canônico: `my-requests`.  
...
| Método | Rota | Descrição |
|--------|------|-----------|
| GET | `/request-lookups/parties` | Busca cliente (SA1) ou fornecedor (SA2), inclusive CNPJ. |
| GET | `/request-lookups/products` | Busca itens SB1. |
| GET | `/request-lookups/products/{code}/warehouse-01-balance` | Saldo informativo almoxarifado 01. |
| GET | `/request-lookups/open-sales-orders` | PV em aberto do cliente na filial (sem membership). |
| GET | `/request-lookups/carriers` | Busca transportadora SA4 (`A4_NREDUZ`). |
...
| GET | `/production/production-order-sets/incomplete` | Conjuntos de OP cujas filhas não batem com a estrutura do produto raiz. |
...
| GET | `/engineering/transformometro/savings-investment/series` | Série Economia bruta vs Investimento do TRANSFORMA+ DELPI (`granularity=day\|month`; pontos com `periodo`, `economia_bruta`, `investimento`; filtros `view`, `filial_id`, `setor_id`, `start_date`, `end_date`). Proxy do `transformometro-api` (`dashboard_evolucao`). |
...
| GET | `/production/appointments/produced-totals` | Total produzido canônico (inspeção final + OP mãe). |
...
**Listagem (`list_kaizen`):** itens com `id`, `annual_savings` (`daily_savings × 253 dias úteis`) e demais campos cadastrais. O dashboard de qualidade usa chamada sem datas para catálogo completo na tabela.
...
annual_savings     = daily_savings × 253             # dias úteis (constante Delpi); validade do kaizen = 1 ano corrido (separado)
```
