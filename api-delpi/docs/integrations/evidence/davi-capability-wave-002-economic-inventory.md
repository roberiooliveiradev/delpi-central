...
- origin/main: `6e10029bcc281c4e0c3575448a1414a157cc3c44`
- Current eligible: **10**
...
- **TIME:** prices[].valid_from is DA1_DATVIG as stored. No start_date/end_date filter exists. Capability is current table rows, not a historical commercial-price series.
...
- **TIME:** Canonical HTTP filters are start_date and end_date. If omitted, backend sets start=today-365d and exclusive end=today+1 (resolve_history_date_range). Date basis is D1_EMISSAO (issue). date_start/date_end are legacy NOT_EXPOSED. Returned start_date / date_end_exclusive are provenance of the actual window.
...
- `RG-PRODUCT-LAST-PURCHASE-VS-HISTORY`: DISTINCT snapshot vs windowed series. last_purchase has no date filter (latest valid NF ever); history defaults to 365 days + history_limit.
...
- **impacto no custo / simular impacto de custo** → `product.cost.impact_simulation` (not: commercial pricing / purchase history)
