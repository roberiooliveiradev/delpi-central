...
| Item | Detalhe | Status |
|------|---------|--------|
| **Cache inspector** | `GET /system/query-cache/stats` — hits/miss por namespace (`stock-value`, `financial-rol`, `production-oee`, `production-otd`, `commercial-rol-series`, `production-oee-series`, `production-otd-series`, LMP, `lmp-summary`) | [x] |
