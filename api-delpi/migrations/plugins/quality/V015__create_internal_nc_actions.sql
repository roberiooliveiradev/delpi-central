CREATE TABLE IF NOT EXISTS quality.internal_nc_actions (
    ...
    CONSTRAINT fk_internal_nc_actions_nonconformity
        FOREIGN KEY (nonconformity_id)
        REFERENCES quality.internal_nonconformities (id)
        ON UPDATE RESTRICT
        ON DELETE CASCADE,

    CONSTRAINT fk_internal_nc_actions_root_cause
        FOREIGN KEY (root_cause_id)
        REFERENCES quality.internal_nc_root_causes (id)
        ON UPDATE RESTRICT
        ON DELETE SET NULL,

    CONSTRAINT ck_internal_nc_actions_status
        CHECK (status IN ('pending', 'in-progress', 'completed', 'cancelled', 'overdue')),

    CONSTRAINT ck_internal_nc_actions_dates
        CHECK (
            start_date IS NULL
            OR due_date IS NULL
    ...
    CONSTRAINT ck_internal_nc_actions_responsible
        CHECK (
            responsible_user_id IS NOT NULL
            OR responsible_external_name IS NOT NULL
        ),

    CONSTRAINT ck_internal_nc_actions_completed_at
        CHECK (
