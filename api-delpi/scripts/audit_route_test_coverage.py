#!/usr/bin/env python3
"""Inventário de cobertura de testes por rota OpenAPI (api-delpi).
...
Regra de covered: operationId aparece como substring em tests/**/*.py
...
COVERAGE_PATH = API_ROOT / "app" / "content" / "route_test_coverage.json"
TESTS_ROOT = API_ROOT / "tests"

# Rotas sem envelope api_delpi_success — isentas do Nível A (meta).
_DEFAULT_EXEMPTS: dict[str, str] = {
    "get_health": "legacy_plain_json_without_envelope",
}
...
def _baseline_operations(baseline: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for op in baseline.get("operations") or []:
        ...
        module = path.strip("/").split("/")[0] if path.strip("/") else "root"
        xd = op.get("xDelpi") if isinstance(op.get("xDelpi"), dict) else {}
        rows.append(
            {
                "operationId": oid,
...
def print_report(coverage: dict[str, Any]) -> None:
    ...
    print(
        f"ops={total} covered={covered} ({pct:.0f}%) gap={gap} exempt={exempt}"
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--check",
        action="store_true",
        help="Falha se inventário não listar o mesmo conjunto de operationIds do baseline",
    )
    parser.add_argument(
        "--check-complete",
        action="store_true",
        help="Falha se houver status=gap (meta 100%)",
    )
    ...
    if args.report or (
        not args.write and not args.check and not args.check_complete
    ):
    ...
    if args.check:
        ...
        if _fingerprint(stored) != _fingerprint(coverage):
            print(
                "Drift no inventário de rotas — rode com --write.",
                file=sys.stderr,
            )
            return 1
        if int(stored.get("operationCount") or 0) != coverage["operationCount"]:
            print("Drift operationCount — rode com --write.", file=sys.stderr)
            return 1
        # Recompute covered/gap from live scan vs stored statuses for covered detection drift
    ...
    if args.check_complete:
        ...
        if gaps:
            print(
                f"Falha — {len(gaps)} rota(s) sem teste (status=gap).",
                file=sys.stderr,
            )
            for oid in gaps[:40]:
