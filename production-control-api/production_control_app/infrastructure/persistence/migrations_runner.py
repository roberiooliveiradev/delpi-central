...
class MigrationError(RuntimeError):
    """Erro de execução de migrations do Portal PCP."""


def ensure_migrations_table(conn: Any) -> None:
    with conn.cursor() as cur:
        cur.execute(f'CREATE SCHEMA IF NOT EXISTS "{PC_SCHEMA_NAME}";')
        cur.execute(
            f"""
            CREATE TABLE IF NOT EXISTS "{PC_SCHEMA_NAME}".schema_migrations (
...
def get_applied_migrations(conn: Any) -> dict[str, dict[str, Any]]:
    with conn.cursor() as cur:
        cur.execute(
            ...
            FROM "{PC_SCHEMA_NAME}".schema_migrations
...
def apply_migration(conn: Any, path: Path) -> None:
    ...
    try:
        with conn.cursor() as cur:
            ...
            cur.execute(
                f"""
                INSERT INTO "{PC_SCHEMA_NAME}".schema_migrations (version, name, checksum)
                VALUES (%s, %s, %s)
                """,
...
def run_migrations() -> None:
    ...
    with get_connection() as conn:
        ...
        if not pending:
            print("[production-control] Nenhuma migration pendente.")
            return

        print("[production-control] Executando migrations...")
        for path in pending:
            apply_migration(conn, path)
        print("[production-control] Migrations aplicadas com sucesso.")
...
def show_status() -> None:
    ...
    with get_connection() as conn:
        ...
        print("Status das migrations do Portal PCP:")
        for path in files:
            version, name = parse_version_and_name(path)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Runner de migrations Postgres do production-control-api.",
    )
