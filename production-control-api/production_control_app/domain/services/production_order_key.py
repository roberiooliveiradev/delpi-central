"""Chave da OP Protheus (C2_OP / H8_OP) e identificador de conjunto (C2_NUM).

Espelha a convenção Delpi de ``api-delpi`` ``protheus_production_orders``:
C2_OP = C2_NUM (6) + C2_ITEM (2) + C2_SEQUEN (3) = 11 posições.

Conjunto = todas as ordens cujo ``production_order`` compartilha o mesmo C2_NUM
(os 6 primeiros dígitos). Ex.: ``10840401003`` e ``10840402001`` → conjunto ``108404``.
"""
...
def conjunto_key_from_order(production_order: str | None) -> str | None:
    """Extrai o C2_NUM (6 primeiros caracteres) da chave completa da OP."""
...
def order_belongs_to_conjunto(production_order: str | None, conjunto_key: str | None) -> bool:
    """True se a OP pertence ao conjunto (mesmo C2_NUM de 6 posições)."""
