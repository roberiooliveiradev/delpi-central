...
@lru_cache(maxsize=1)
def _public_cockpit_settings() -> dict[str, Any]:
    content = json.loads(_CONTENT_PATH.read_text(encoding="utf-8"))
    settings = content.get("publicCockpit")
    return settings if isinstance(settings, dict) else {}


class PublicCockpitAccessService:
    """Valida o token do link público do operador (hoje um slug aberto configurável)."""

    def token(self) -> str:
        value = str(_public_cockpit_settings().get("token") or "").strip()
    ...
    def message(self, key: str, default: str) -> str:
        messages = _public_cockpit_settings().get("messages")
