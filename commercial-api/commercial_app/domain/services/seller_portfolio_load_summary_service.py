from __future__ import annotations

from collections import defaultdict
from typing import Mapping, Sequence

from commercial_app.domain.entities.portfolio_load import (
    PersonLoadItem,
    PortfolioLoadItem,
    PortfolioLoadSummary,
    TotvsLoadMetricsStatus,
)
from commercial_app.domain.entities.seller_portfolio import SellerPortfolio
from commercial_app.domain.ports.open_orders_metrics_port import CustomerOpenOrderMetric
from commercial_app.domain.services.seller_portfolio_coverage_audit_service import (
    customer_coverage_key,
)

_TOTVS_UNAVAILABLE_REASON = "open_orders_aggregation_not_wired"
_TOTVS_FETCH_FAILED_REASON = "open_orders_metrics_fetch_failed"


def _normalize(value: str) -> str:
    return str(value or "").strip()


def _member_ids(portfolio: SellerPortfolio) -> list[str]:
    from_members = [
        _normalize(member.user_id)
        for member in portfolio.members
        if _normalize(member.user_id)
    ]
    if from_members:
        seen: set[str] = set()
        ordered: list[str] = []
        for user_id in from_members:
            if user_id in seen:
                continue
            seen.add(user_id)
            ordered.append(user_id)
        return ordered
    owner = _normalize(portfolio.owner_user_id)
    return [owner] if owner else []


def _unique_customer_keys(portfolio: SellerPortfolio) -> set[tuple[str, str]]:
    seen: set[tuple[str, str]] = set()
    for customer in portfolio.customers:
        key = customer_coverage_key(customer.customer_code, customer.customer_store)
        if not key[0] or not key[1]:
            continue
        seen.add(key)
    return seen


def _metrics_index(
    metrics: Sequence[CustomerOpenOrderMetric] | None,
) -> dict[tuple[str, str], CustomerOpenOrderMetric]:
    index: dict[tuple[str, str], CustomerOpenOrderMetric] = {}
    for item in metrics or ():
        key = customer_coverage_key(item.customer_code, item.customer_store)
        if key[0] and key[1]:
            index[key] = item
    return index


def _sum_open_value(
    keys: set[tuple[str, str]],
    index: Mapping[tuple[str, str], CustomerOpenOrderMetric],
) -> float:
    total = 0.0
    for key in keys:
        metric = index.get(key)
        if metric is not None:
            total += float(metric.open_value or 0)
    return total


def _attention_count(
    keys: set[tuple[str, str]],
    index: Mapping[tuple[str, str], CustomerOpenOrderMetric],
) -> int:
    return sum(1 for key in keys if index.get(key) and index[key].has_overdue)


class SellerPortfolioLoadSummaryService:
    """KPIs de carga por carteira e por pessoa (E6.2).

    customer_count / member_count vêm do estado Delpi (Postgres).
    open_value / attention_count vêm da agregação TOTVS (api-delpi) quando disponível.
    """

    def summarize(
        self,
        portfolios: Sequence[SellerPortfolio],
        *,
        metrics: Sequence[CustomerOpenOrderMetric] | None = None,
        totvs_available: bool | None = None,
        totvs_reason: str | None = None,
    ) -> PortfolioLoadSummary:
        index = _metrics_index(metrics)
        use_totvs = bool(totvs_available) if totvs_available is not None else bool(index)
        if totvs_available is False:
            use_totvs = False
        reason = totvs_reason
        if not use_totvs and reason is None:
            reason = _TOTVS_UNAVAILABLE_REASON

        portfolio_items: list[PortfolioLoadItem] = []
        by_user_portfolios: dict[str, list[SellerPortfolio]] = defaultdict(list)

        for portfolio in portfolios:
            member_ids = _member_ids(portfolio)
            customer_keys = _unique_customer_keys(portfolio)
            open_value = _sum_open_value(customer_keys, index) if use_totvs else None
            attention = _attention_count(customer_keys, index) if use_totvs else None
            portfolio_items.append(
                PortfolioLoadItem(
                    id=portfolio.id,
                    display_name=portfolio.display_name,
                    active=portfolio.active,
                    customer_count=len(customer_keys),
                    member_count=len(member_ids),
                    open_value=open_value,
                    attention_count=attention,
                )
            )
            for user_id in member_ids:
                by_user_portfolios[user_id].append(portfolio)

        person_items: list[PersonLoadItem] = []
        for user_id in sorted(by_user_portfolios.keys()):
            person_portfolios = by_user_portfolios[user_id]
            customer_keys: set[tuple[str, str]] = set()
            portfolio_ids: list[str] = []
            for portfolio in person_portfolios:
                portfolio_ids.append(portfolio.id)
                customer_keys |= _unique_customer_keys(portfolio)
            person_items.append(
                PersonLoadItem(
                    user_id=user_id,
                    portfolio_ids=tuple(portfolio_ids),
                    portfolio_count=len(portfolio_ids),
                    customer_count=len(customer_keys),
                    open_value=_sum_open_value(customer_keys, index) if use_totvs else None,
                    attention_count=(
                        _attention_count(customer_keys, index) if use_totvs else None
                    ),
                )
            )

        return PortfolioLoadSummary(
            portfolios=tuple(portfolio_items),
            by_person=tuple(person_items),
            totvs_metrics=TotvsLoadMetricsStatus(
                available=use_totvs,
                reason=None if use_totvs else reason,
            ),
        )
