...
COMMERCIAL_SCHEMA_NAME = "commercial"
MIGRATIONS_DIR = Path(__file__).resolve().parents[4] / "migrations"


class MigrationError(RuntimeError):
    """Erro de execução de migrations da Commercial API."""
...
def ensure_migrations_table(conn: Any) -> None:
    with conn.cursor() as cur:
        cur.execute(f'CREATE SCHEMA IF NOT EXISTS "{COMMERCIAL_SCHEMA_NAME}";')
        cur.execute(
            f"""
            CREATE TABLE IF NOT EXISTS "{COMMERCIAL_SCHEMA_NAME}".schema_migrations (
...
def get_applied_migrations(conn: Any) -> dict[str, dict[str, Any]]:
    with conn.cursor() as cur:
        cur.execute(
            ...
            FROM "{COMMERCIAL_SCHEMA_NAME}".schema_migrations
...
def apply_migration(conn: Any, path: Path) -> None:
    ...
    try:
        with conn.cursor() as cur:
            ...
            cur.execute(
                f"""
                INSERT INTO "{COMMERCIAL_SCHEMA_NAME}".schema_migrations (version, name, checksum)
                VALUES (%s, %s, %s)
                """,
...
def run_migrations() -> None:
    ...
    with get_connection() as conn:
        ...
        if not pending:
            print("[commercial] Nenhuma migration pendente.")
            return

        print("[commercial] Executando migrations...")
        for path in pending:
            apply_migration(conn, path)

        print("[commercial] Migrations aplicadas com sucesso.")
...
def show_status() -> None:
    ...
    with get_connection() as conn:
        ...
        print("Status das migrations da Commercial API:")
        for path in files:
            version, name = parse_version_and_name(path)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Runner de migrations Postgres da Commercial API.",
    )
    parser.add_argument(
        "command",
        choices=["up", "status"],
