...
def test_list_customers_in_scope_returns_membership_items() -> None:
    ...
    assert scope_svc.execute.call_args.kwargs["portfolio_ids"] == ["p1"]
...
def test_list_customers_in_scope_csv_seller_ids() -> None:
    scope = CommercialCustomerScope(
        unrestricted=False,
    ...
    with (
        ...
        customer_routes.list_customers_in_scope(
            request,
            seller_id="p1,p2",
            portfolio_id=None,
        )

    assert scope_svc.execute.call_args.kwargs["portfolio_ids"] == ["p1", "p2"]
